// date-trio.ts — QUÉ PARTE DE UNA FECHA ES CADA COMBO, Y QUÉ VALOR LE VA.
//
...
//
//   · `pkg/autofill.ts` (una `r`) corre DENTRO de la página, inyectado — usa `injectableSource()`.
//   · `pkg/autofill-qr.ts` (dos `r`) maneja Playwright desde afuera — importa las funciones.
//
...
//       espacios. Hoy sobrevive; `date-trio.spec.ts` lo comprueba evaluando la fuente generada, así
