// autofill.ts — RELLENA LOS INPUTS DE LA PANTALLA para que manejar el wizard a mano sea dar
// «Continuar» y nada más.
//
// POR QUÉ EXISTE, y por qué no alcanzaba con lo que ya había. El guiado (`bin/advisor <m> auto`) siembra
...
/** Los datos, de la misma cadena de env que usa `bin/advisor`. */
 ...
 * (`pkg/date-trio.ts`)—; el segundo es el guion, que la usa. Van separados porque `addInitScript(fn)`
...
function script(data: AutofillData) {
    ...
    /* LA REGLA DE FECHA VIENE DEL MÓDULO COMPARTIDO (`pkg/date-trio.ts`), inyectado como
    ...
    async function fill(): Promise<number> {
         ...
         * está en lo que MUESTRAN — y eso lo decide ahora la regla compartida (`pkg/date-trio.ts`),
