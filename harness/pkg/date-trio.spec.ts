// Pruebas de `pkg/date-trio.ts` — la regla compartida por los DOS autorrellenos.
//
...
//     npx playwright test pkg/date-trio.spec.ts
...
test('la regla llega al navegador y decide igual que en Node', async ({ browser }) => {
      const { installAutofill } = await import('./autofill.ts');
