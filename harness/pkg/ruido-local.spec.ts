...
import { isLocalNoise } from './wizard-navegador.ts';
...
test.describe('el filtro de ruido de local', () => {
      ...
      test('descarta la hidratación del nonce y deja pasar cualquier otra', () => {
            const fromNonce = 'A tree hydrated but some attributes of the server rendered HTML didn\'t match'
                  + ' the client properties. <script + nonce="" - nonce="yhopWL14+9f/xNUx4omRiQ==" >';
            const fromOneField = 'A tree hydrated but some attributes of the server rendered HTML didn\'t match'
                  + ' the client properties. <input + value="" - value="3131010101" >';

            expect(isLocalNoise(fromNonce)).toBe(true);
            expect(isLocalNoise(fromOneField)).toBe(false);
      });
      ...
      test('reconoce el nonce aunque venga al final de un mensaje largo', () => {
            const length = 'A tree hydrated but some attributes didn\'t match. ' + 'x'.repeat(1200) + ' nonce="abc"';

            expect(isLocalNoise(length)).toBe(true);
      });

      test('el WebSocket de Echo que no resuelve en local es ruido', () => {
            expect(isLocalNoise('WebSocket connection to \'wss://ws.credito/app/x\' failed: ERR_NAME_NOT_RESOLVED')).toBe(true);
      });

      // 🔴 Lo que NUNCA puede filtrarse: un error de verdad del producto.
      test('un error real del producto no es ruido', () => {
            expect(isLocalNoise('Uncaught TypeError: Cannot read properties of undefined (reading \'value\')')).toBe(false);
            expect(isLocalNoise('No routes matched location "/self-service/abc/continue"')).toBe(false);
            expect(isLocalNoise('The API version "5.4.296" does not match the Worker version "5.4.449"')).toBe(false);
      });
});
