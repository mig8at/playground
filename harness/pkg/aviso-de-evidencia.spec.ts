...
import { avisoDeEvidencia } from './wizard-navegador.ts';
...
test.describe('el aviso de evidencia de un caso que cerró', () => {
      test('sin nada que decir, no dice nada', () => {
            expect(avisoDeEvidencia([], [])).toEqual([]);
      });
      ...
      test('cuenta consola y red por separado', () => {
            const lineas = avisoDeEvidencia(['error: a', 'error: b', 'error: c'], ['HTTP 500 GET /x', 'HTTP 404 GET /y']);

            expect(lineas[0]).toContain('3 de consola y 2 de red');
      });

      test('con una sola fuente, no inventa la otra', () => {
            expect(avisoDeEvidencia(['error: a'], [])[0]).toContain('1 de consola');
            expect(avisoDeEvidencia(['error: a'], [])[0]).not.toContain('de red');
            expect(avisoDeEvidencia([], ['HTTP 500 GET /x'])[0]).toContain('1 de red');
      });
      ...
      test('invita a mirar, no declara un fallo', () => {
            const lineas = avisoDeEvidencia(['error: a'], []);

            expect(lineas[0]).toContain('cerró');
            expect(lineas[0]).toContain('no cambia el veredicto');
            expect(lineas[0]).not.toMatch(/fall[óo]|error del caso/i);
      });
      ...
      test('muestra unas pocas líneas, no el volcado entero', () => {
            const muchas = Array.from({ length: 30 }, (_, i) => `error: ${i}`);

            expect(avisoDeEvidencia(muchas, []).length).toBe(1 + 3);
            expect(avisoDeEvidencia(muchas, [], 1).length).toBe(1 + 1);
      });
});
