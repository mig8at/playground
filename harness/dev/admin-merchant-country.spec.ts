 ...
 *   npx playwright test dev/admin-merchant-country.spec.ts --reporter=list
 *
...
test.beforeEach(async ({ context }) => {
      // Sin contraseña: `bin/admin-session` emite la cookie de sesión directamente. Se pasa `url` y no
      // `domain` porque Chromium descarta un dominio con punto inicial (`.localhost` es sufijo público).
      const output = execFileSync(join(process.cwd(), 'bin/admin-session'), { encoding: 'utf8' });
