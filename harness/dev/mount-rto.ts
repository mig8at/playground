// mount-rto.ts — deja el lender Rent to Own USABLE en local, con su rama de codeudor.
//
...
//
// Uso:  node dev/mount-rto.ts            (idempotente: no pisa lo que ya exista)
...
console.log(`
  Falta a mano, y no lo hace este script:
    ...
    · el AML del codeudor — en local NO corre para nadie (cero filas en toda la base) y su
      endpoint está tras auth de sesión. Se inyecta con dev/inject-aml.ts <user_id>.
`);
await close();
