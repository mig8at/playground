...
//
// QUÉ SEED: lo mismo que `caso.ts` y el panel, y por la misma razón —el buró no lo contesta el
...
const { branchDocument, branchPhone, syntheticPhone } = await import('../pkg/telefonos.ts');
...
const { validationErrors: screenErrors } = await import('../pkg/autorrelleno.ts');
...
async function correr(c: Case, i: number): Promise<Result> {
    ...
    if (FLOW === 'merchant') {
        ...
        try {
            const { preflightBranch, mismatchNotice, advisorSubject } = await import('../pkg/preflight-sucursal.ts');
