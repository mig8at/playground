...
const { lokiConfig, whyNot } = await import('../pkg/loki.ts');

/** ¿La URL/host apunta a la máquina local? */
const isLocal = (v: string) => /(^|\/\/)(localhost|127\.0\.0\.1|::1)(:|\/|$)/.test(v.trim());

type Check = { clave: string; valor: string; local: boolean; nota?: string };
...
const checkList: Check[] = checks.map((c) => ({
    ...
    local: TARGET !== 'local' && isLocal(c.valor),
}));
...
// La regla de "¿se puede consultar?" NO se reimplementa acá: se le pregunta a `whyNot()`, que es la que
...
const badList = checkList.filter((c) => c.local);
...
function tsFiles(dir: string, out: string[] = []): string[] {
    ...
    for (const e of entries) {
        const rel = `${dir}/${e}`;
        if (statSync(join(ROOT, rel)).isDirectory()) { if (e !== 'node_modules') tsFiles(rel, out); }
...
export function outsideChain(): Array<{ archivo: string; linea: number; clave: string }> {
    const keys = chainKeys();
    const foundOnes: Array<{ archivo: string; linea: number; clave: string }> = [];
    for (const arch of ['pkg', 'dev', 'bin', 'panel', 'channel'].flatMap((d) => tsFiles(d))) {
        const lines = readFileSync(join(ROOT, arch), 'utf8').split('\n');
        lines.forEach((l, i) => {
            const withoutComment = l.replace(/\/\/.*$/, '').replace(/\/\*[\s\S]*?\*\//g, '');
            for (const m of withoutComment.matchAll(/process\.env\.([A-Z0-9_]+)|process\.env\[['"`]([A-Z0-9_]+)['"`]\]/g)) {
                const key = m[1] ?? m[2];
                // `env.ts` ES la cadena: es el único que tiene que leer `process.env`.
                if (!keys.has(key) || arch === 'pkg/env.ts') continue;
                ...
                const rest = withoutComment.slice((m.index ?? 0) + m[0].length);
                if (/^\s*(=[^=]|\+=|\?\?=|\|\|=)/.test(rest)) continue;
                foundOnes.push({ archivo: arch, linea: i + 1, clave: key });
            }
        });
    }
    return foundOnes;
...
if (isCLI) {
    ...
    console.log('');
    for (const i of informative) console.log(`  · ${i.clave.padEnd(38)} ${i.valor}`);

    if (outside.length) {
        console.log(`\n  ✗ ${outside.length} lectura(s) del ambiente FUERA de la cadena:`);
        for (const f of outside) console.log(`      ${f.archivo}:${f.linea}  process.env.${f.clave}`);
    ...
    if (badList.length) {
        console.log(`\n  ✗ ${badList.length} valor(es) apuntan a localhost con target '${TARGET}'.`);
