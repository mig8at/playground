...
//
// Existe porque `bin/advisor` greppeaba `.env.$TARGET` a mano: eso ignora los hechos COMPARTIDOS de
// `env/<target>.env` y, con la herencia de targets (staging → dev), quedaría siempre corto.
//
//   node bin/env-get.ts E2E_BASE_URL [fallback]
import { env } from '../pkg/env.ts';

const [key, fallback = ''] = process.argv.slice(2);
if (!key) {
    console.error('uso: node bin/env-get.ts <CLAVE> [fallback]');
    process.exit(2);
}
