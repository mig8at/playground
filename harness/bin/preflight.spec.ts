...
import { fueraDeLaCadena } from './preflight.ts';

test('nadie resuelve el ambiente por fuera de la cadena', () => {
      const fuera = fueraDeLaCadena();
      const detalle = fuera.map((f) => `${f.archivo}:${f.linea}  process.env.${f.clave}`).join('\n');
      expect(fuera, `\n${detalle}\n\n`
            ...
            + 'si es el backend, o `subDelAsesor()` si es el asesor.\n').toEqual([]);
});
...
test('no marca las ESCRITURAS, que son la forma legítima de un override', () => {
      const archivos = fueraDeLaCadena().map((f) => f.archivo);
      expect(archivos).not.toContain('pkg/ecommerce.ts');   // `process.env.E2E_WEBHOOK_URL = opciones.processUrl`
      expect(archivos).not.toContain('dev/ecommerce.ts');   // fija sus destinos de prueba
});
...
test('el chequeo sabe de las claves que existen hoy, no de una lista escrita a mano', () => {
      const fuente = readFileSync(new URL('./preflight.ts', import.meta.url), 'utf8');
      expect(fuente).toContain('readdirSync(RAIZ)');
      expect(fuente).not.toMatch(/const CLAVES\s*=\s*\[/);
});
