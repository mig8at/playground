...
async function profile(userRequestId: number) {
    ...
    const dc = parse(summary?.datacredito);
    const abaco = parse(summary?.abaco);

    const monthlyIncomeAmount = Number(income) || null;
    // el ingreso INFORMAL es el que midió Ábaco (average_income, persistido por AbacoParserService)
    const informalIt = Number((abaco as { average_income?: unknown }).average_income) || null;
    ...
    const negatives = principals ? Number(principals.currentNegativeCredits) || 0 : null;
    ...
    return {
        ...
        monthlyInformalIncomeAmount: informalIt,
        monthlyIncomeAmount,
        creditScore,
