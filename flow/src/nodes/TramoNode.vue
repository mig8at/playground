...
import { ui, state, money, tramosOf, tramoIsOn, setTramoOn, addTramo, removeTramo, setTramo, activeTramoIndex, openFieldInfo, productionCollectionChanged } from '../store'
...
const montoFmt = computed(() => money(Number(String(state.monto).replace(/\D/g, '') || 0)))
const localTramos = computed(() => productionCollectionChanged(name.value, 'tramos', tramos.value))
</script>

<template>
  <div class="node node--tramo prov-node" :class="{ 'node--prodlocal': localTramos }">
    ...
    <div class="node__body">
      <template v-if="on">
        ...
        <div v-for="(t, i) in tramos" :key="i" class="tr-row" :class="{ 'tr-row--on': i === activeIdx }">
          <AffixField currency class="afld--mny tr-f"><MoneyInput class="afld__in" :model-value="t.min" @update:model-value="v => setTramo(name, i, 'min', v)" /></AffixField>
          <AffixField currency class="afld--mny tr-f"><MoneyInput class="afld__in" :model-value="t.max" @update:model-value="v => setTramo(name, i, 'max', v)" /></AffixField>
