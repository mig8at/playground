...
const pinta = (m: Record<string, Res>) => {
    for (const c of CENTRALES) {
        const r = m[c];
        console.log(`      ${c.padEnd(24)} HTTP ${r.status} · ${r.code}${r.code === OMITIDO ? '  ← NO se consulta (omitido por flujo)' : ''}`);
    }
};
...
// ── veredicto ────────────────────────────────────────────────────────────────────────────────────
const cambiaron = CENTRALES.filter((c) => antes[c].code !== OMITIDO && despues[c].code === OMITIDO);
const yaEstaban = CENTRALES.filter((c) => antes[c].code === OMITIDO);
const faltan = CENTRALES.filter((c) => despues[c].code !== OMITIDO);
...
if (sinRespuesta || able.status === 0) {
...
} else if (!faltan.length && cambiaron.length) {
    code = 0;
    texto = `✓ LA FIRMA APAGA LA CONSULTA: las ${CENTRALES.length} variaciones de Experian pasaron a ${OMITIDO}.\n`
        + `    ${cambiaron.length} cambiaron con la firma${yaEstaban.length ? ` y ${yaEstaban.length} ya venían omitidas` : ''}.`;
    ...
    texto = `✗ LA FIRMA NO APAGA LA CONSULTA: siguen pidiendo buró → ${faltan.join(', ')}.\n`
