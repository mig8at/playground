...
`pre-approvals-service`, más el harness de acá) con una migración a medio camino, decenas de
...
2. **Harness** que ejercen el flujo real punta a punta inyectando el KYC/buró sintético, para no depender
   de Experian ni de un cliente real (`harness/`).
...
| **Probar un flujo punta a punta con navegador** | `harness/` | `cd harness && npm run dev` → panel en **:5195**. Maneja el wizard real (`:5174`) con Playwright. |
| **Probar el flujo sin navegador** (API + BD, rápido y repetible) | `harness/` | `node dev/sweep.ts close <comercio> <lenderId>` — recorre el cierre endpoint por endpoint, traza contra la BD y su exit code es el veredicto. |
| **Depurar un muro del harness** (pantalla en blanco, 500, no lista la entidad) | `context/` → `harness/` | Primero [`findings`](context/server/data/flows/findings/doc.md); si no está, la sección de gotchas del [README de harness](harness/README.md). |
| **Fabricar un caso / inyectar buró y KYC sintético** | `harness/` | El panel (`npm run dev`) define el usuario sintético; por CLI, `node bin/dbops.ts synth-fill <uReq>`. |
...
| **Entender el canal ecommerce** (contrato base64, checkout de tienda) | `creditop-woocommerce/` | El plugin es el emisor original del contrato. Port en TS: `harness/pkg/ecommerce.ts`. |
...
| [`harness/`](harness/README.md) | Harness **Playwright + TypeScript** que maneja el wizard real de originación (`:5174`) de punta a punta, con KYC/buró sintético, panel visual (**:5195**), flota de 8 mocks locales y barrido headless por API. |
...
| Puerto | Qué | Nota |
|---|---|---|
| 5174 | wizard `loan-request-wizard` | **no vive acá** — es `frontend-monorepo`; lo levanta el harness |
...
| 5193 | `context` (viz) | |
| 5195 | `harness` (panel) | override con `PANEL_PORT` |
| 5196 | `engine` | **`strictPort: true`** (igual que `flow`). Override: `PORT=5197 npm run dev` |
| 8095–8102 | mocks de `harness` | preapprovals 8095 · redirect 8096 · payvalida 8097 · mdm 8098 · lenders 8099 · pdf-mapper 8100 · forms 8101 · abaco 8102 |
...
`harness` exige `I_KNOW_THIS_TOUCHES_SHARED_DEV=1` para escribir fuera de local, y no es burocracia:
cambiar de comercio en `harness` **escribe en la BD**, y cada arranque hace un scrub que borra el
...
- **[`harness/README.md`](harness/README.md) (raíz) está PODRIDO.** Documenta una interfaz de `bin/asesor` que **ya no
  ...
  en `harness/bin/asesor`. La interfaz real es **sin flags**:
  `bin/asesor <comercio> [auto [success|rejected|pending]]` — y el comercio va **primero** (el script
  rechaza `bin/asesor auto` con un error explícito). Usá el [README de harness](harness/README.md),
...
- **`npm run test:onboarding` (harness) apunta a `tests/onboarding`, carpeta que no existe.** Script
...
| Archivo | Qué aporta |
|---|---|
| [`harness/README.md`](harness/README.md) | Demos visuales del wizard desde `harness`. **Podrido** (ver arriba): los flags ya no existen. |
