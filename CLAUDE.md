...
| **¿cómo es el diseño de este flujo?** (Figma) · **pasar una pantalla a código** | **por consola, con ids de Figma** (`<clave del archivo>/<nodo>`, o la URL de Figma): `make visor-pantalla R=<clave/nodo>` (el paquete: textos, destinos, imágenes, componentes, tokens y el HTML) → `make visor-recursos R=… DIR=…` (las imágenes originales). ¿Cuánto confiar en ese HTML? `make visor-fidelidad R=<clave/nodo>`. ¿Miguel señaló una capa (un enlace con `?capa=`)? `make visor-capa R='<enlace>'`. El flujo con sus ids: `make visor-pantallas P=<clave>`; tokens y componentes: `make visor-tokens P=…` · `make visor-componentes P=…`. Sin id, `make visor-buscar Q='…'` busca por lo que dice la pantalla. `make visor` es la interfaz, para mirar. ¿Sigue igual la pantalla que enlaza una tarea? `make visor-enlaces` |
