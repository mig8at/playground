...
| **¿cómo es el diseño de este flujo?** (Figma) | `bin/pg figma map '<url de la sección>'` por consola, o `make visor` para recorrerlo pantalla por pantalla con el prototipo navegable. Una pantalla: `bin/pg figma node '<url>' --id <id> --depth 8 --text`. **Para pasarlo a código**: `bin/pg figma tokens '<url>' --css` (o `--tailwind`), los colores y textos con su nombre del sistema de diseño. ¿Sigue igual la pantalla que enlaza una tarea? `make visor-enlaces` |
