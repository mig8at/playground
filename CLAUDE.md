...
| **¿cómo es el diseño de este flujo?** (Figma) | `bin/pg figma map '<url de la sección>'` por consola, o `make visor` para recorrerlo pantalla por pantalla con el prototipo navegable. Una pantalla: `bin/pg figma node '<url>' --id <id> --depth 8 --text`. ¿Sigue igual la pantalla que enlaza una tarea? `make visor-enlaces` |
