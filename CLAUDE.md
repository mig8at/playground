...
| **¿cómo es el diseño de este flujo?** (Figma) · **pasar una pantalla a código** | **por consola**: `make visor-buscar Q='…'` (qué pantalla es) → `make visor-pantalla R=<proyecto/pantalla>` (el paquete: textos, destinos, imágenes, componentes, tokens y el HTML) → `make visor-recursos R=… DIR=…` (las imágenes originales). Tokens y componentes del flujo: `make visor-tokens P=…` · `make visor-componentes P=…`. `make visor` es la interfaz, para mirar. ¿Sigue igual la pantalla que enlaza una tarea? `make visor-enlaces` |
