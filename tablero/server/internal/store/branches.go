...
// DefaultEnvironments son las ramas que cuentan como "ambiente": si el commit está acá, está desplegado
// (o en camino). El orden es de menor a mayor riesgo, que es como conviene leerlas.
var DefaultEnvironments = []string{"develop", "staging", "qa", "main"}
...
// reBranchPattern valida CADA patrón del frontmatter. Se acota a propósito: es una subcadena de nombre de
// rama, no una expresión — un patrón libre acabaría matcheando ramas ajenas y la card mentiría al revés.
var reBranchPattern = regexp.MustCompile(`^[\w][\w./-]{2,}$`)

// patternsOf parte el valor del frontmatter en la LISTA de subcadenas a buscar, separadas por coma.
//
...
func patternsOf(v string) []string {
	var out []string
	for _, p := range strings.Split(v, ",") {
		p = strings.TrimSpace(p)
		if reBranchPattern.MatchString(p) {
			out = append(out, p)
		}
	}
	return out
...
// MeasureBranches mide, para cada (id de tarea → patrón), las ramas que matchean en los repos bajo `root`.
//
// `environments` puede venir vacío y usa los de por defecto. El contexto acota el tiempo total: son
// muchas invocaciones de git y esto lo dispara una persona esperando en la terminal.
func MeasureBranches(ctx context.Context, root string, patterns map[string]string, environments []string) BranchSnapshot {
	if len(environments) == 0 {
		environments = DefaultEnvironments
	}
	snap := BranchSnapshot{MeasuredAt: time.Now().Format(time.RFC3339), Root: root, Tasks: map[string]TaskBranches{}}

	repos := reposIn(root)
	prs := &prCache{byRepo: map[string]map[string]*PullRequest{}}
	...
	for id, pattern := range patterns {
		pats := patternsOf(pattern)
		if len(pats) == 0 {
			continue
		}
		wg.Add(1)
		go func(id, pattern string, pats []string) {
			defer wg.Done()
			queue <- struct{}{}
			defer func() { <-queue }()
			if ctx.Err() != nil {
				mu.Lock()
				snap.Incomplete = append(snap.Incomplete, id)
				mu.Unlock()
				return
			}
			res := measureTask(ctx, repos, pattern, pats, environments, prs)
			res.MeasuredAt = time.Now().Format(time.RFC3339)
			mu.Lock()
			defer mu.Unlock()
			if ctx.Err() != nil {
	...
	wg.Wait()
	sort.Strings(snap.Incomplete)
	return snap
}

// measureTask es el trabajo de UNA tarea: sus ramas en cada repo, hasta dónde llegaron y su PR.
func measureTask(ctx context.Context, repos []string, pattern string, pats, environments []string, prs *prCache) TaskBranches {
	{
		res := TaskBranches{Pattern: pattern}
		for _, repo := range repos {
			...
			for _, pat := range pats {
				for _, rm := range matchingBranches(ctx, repo, pat) {
					if !seen[rm.Name] {
						seen[rm.Name] = true
						branches = append(branches, rm)
					}
				}
			}
			for _, rm := range branches {
				...
				if out, err := git(ctx, repo, "log", "-1", "--format=%h|%s", ref); err == nil {
					if h, subject, ok := strings.Cut(strings.TrimSpace(out), "|"); ok {
						r.Commit, r.Subject = h, subject
					}
				}
		...
		sort.Slice(res.Branches, func(i, j int) bool {
			if res.Branches[i].Repo != res.Branches[j].Repo {
				return res.Branches[i].Repo < res.Branches[j].Repo
			}
			return res.Branches[i].Branch < res.Branches[j].Branch
		})
		return res
	}
}
...
type prCache struct {
	mu     sync.Mutex
	byRepo map[string]map[string]*PullRequest // nil = gh no contestó para ese repo
}

func (c *prCache) of(ctx context.Context, repo, branch string) *PullRequest {
	c.mu.Lock()
	defer c.mu.Unlock()
	m, seen := c.byRepo[repo]
	if !seen {
		m = repoPRs(ctx, repo)
		c.byRepo[repo] = m
	}
	...
	pr := prForBranch(ctx, repo, branch)
	m[branch] = pr
	return pr
}

// reaches contesta, para UNA rama y UN ambiente: ¿el cambio ya está ahí, cuántos commits propios le
...
func reaches(ctx context.Context, repo, ref, env string, pr *PullRequest) (reached bool, own int, how string) {
	...
	out, err := git(ctx, repo, "cherry", "origin/"+env, ref)
	if err != nil {
		return false, 0, ""
	}
	...
	for _, l := range strings.Split(strings.TrimSpace(out), "\n") {
		...
		tipInside = strings.HasPrefix(l, "-")
	}
	if tipInside {
		return true, own, "patch"
	}
	if pr != nil && pr.State == "MERGED" && pr.MergeCommit != "" {
		if _, err := git(ctx, repo, "merge-base", "--is-ancestor", pr.MergeCommit, "origin/"+env); err == nil {
			return true, own, "pr"
		}
	}
	return false, own, "no"
...
// LooseBranch es una rama vista en un repo, sin atarla a ninguna tarea. Es la materia prima de la
...
func AllBranches(ctx context.Context, root string) []LooseBranch {
	var out []LooseBranch
	for _, repo := range reposIn(root) {
		...
		for _, ref := range []string{"refs/remotes/origin", "refs/heads"} {
			...
			for _, l := range strings.Split(txt, "\n") {
				...
				if len(parts) < 2 || parts[0] == "" || strings.HasSuffix(parts[0], "/HEAD") {
					continue
				}
				nom, local := parts[0], ref == "refs/heads"
				if !local {
				...
				out = append(out, LooseBranch{Repo: name, Branch: nom, Date: parts[1], Local: local, Subject: subject})
			}
		}
	}
...
// reposIn lista los repos git bajo root, bajando un nivel extra (en `github/` conviven repos sueltos y
// una carpeta paraguas con un repo por servicio adentro). Mismo criterio que el pulso.
func reposIn(root string) []string {
	var out []string
...
// matchingBranches devuelve las ramas cuyo nombre contiene el patrón: primero las REMOTAS (sin el
...
func matchingBranches(ctx context.Context, repo, pattern string) []branch {
	var out []branch
	seen := map[string]bool{}
	if txt, err := git(ctx, repo, "for-each-ref", "--format=%(refname:short)", "refs/remotes/origin"); err == nil {
		for _, l := range strings.Split(txt, "\n") {
			...
			trimmed := strings.TrimPrefix(l, "origin/")
			if strings.Contains(trimmed, pattern) && !seen[trimmed] {
				seen[trimmed] = true
				out = append(out, branch{Name: trimmed})
			}
		}
	}
	if txt, err := git(ctx, repo, "for-each-ref", "--format=%(refname:short)", "refs/heads"); err == nil {
		for _, l := range strings.Split(txt, "\n") {
			l = strings.TrimSpace(l)
			if l == "" || !strings.Contains(l, pattern) || seen[l] {
				continue
			}
			seen[l] = true
			out = append(out, branch{Name: l, Local: true})
		}
	}
	return out
}

// branch es el resultado interno de la búsqueda: el nombre y si sólo existe en esta máquina.
type branch struct {
	Name  string
	Local bool
}

// SaveBranchSnapshot escribe el snapshot. Va a `data/cache/` porque es descartable: se regenera
// midiendo de nuevo, y por eso está fuera de git (igual que el snapshot del sprint y el pulso).
func SaveBranchSnapshot(dir string, s BranchSnapshot) error {
...
// ReadBranchSnapshot devuelve el snapshot guardado. Si no existe, devuelve uno vacío sin error: no
// haber medido todavía es un estado normal, no una falla — la card lo dice y ofrece medir.
func ReadBranchSnapshot(dir string) BranchSnapshot {
	var s BranchSnapshot
	b, err := os.ReadFile(filepath.Join(dir, "ramas.json"))
	if err != nil {
		return BranchSnapshot{Tasks: map[string]TaskBranches{}}
	}
	if err := json.Unmarshal(b, &s); err != nil || s.Tasks == nil {
		return BranchSnapshot{Tasks: map[string]TaskBranches{}}
	}
	return s
}
...
// reRemote saca `owner/repo` de la URL del remoto. Cubre las tres formas que aparecen en estos repos:
...
var reRemote = regexp.MustCompile(`[:/]([\w.-]+)/([\w.-]+?)(?:\.git)?$`)
...
func ownerRepo(ctx context.Context, dir string) string {
	...
	m := reRemote.FindStringSubmatch(strings.TrimSpace(out))
	if m == nil {
		return ""
	}
	return m[1] + "/" + m[2]
...
// repoPRs pide los PRs de un repo en UNA llamada y los indexa por rama de origen. Devuelve nil si no
...
// vuelven a preguntar una por una con `prForBranch`: pocas llamadas, y sólo para los huecos.
func repoPRs(ctx context.Context, dir string) map[string]*PullRequest {
	slug := ownerRepo(ctx, dir)
	if ghBin == "" || slug == "" {
		return nil
	}
	rawItems, ok := ghPRs(ctx, dir, slug, "--limit", "200")
	if !ok {
		return nil
	}
	return indexPRs(rawItems)
}

// prForBranch busca el PR de UNA rama, para las que la ventana de `repoPRs` no alcanzó. Devuelve nil si
...
func prForBranch(ctx context.Context, dir, branch string) *PullRequest {
	slug := ownerRepo(ctx, dir)
	if ghBin == "" || slug == "" {
		return nil
	}
	rawItems, ok := ghPRs(ctx, dir, slug, "--search", "head:"+branch, "--limit", "20")
	if !ok {
		return nil
	}
	return indexPRs(rawItems)[branch]
}
...
func ghPRs(ctx context.Context, dir, slug string, extra ...string) ([]rawPR, bool) {
	...
	var rawItems []rawPR
	if err := json.Unmarshal([]byte(sb.String()), &rawItems); err != nil {
		return nil, false
	}
	return rawItems, true
}

// indexPRs pasa de la lista de gh al mapa rama → PR. Si una rama tuvo VARIOS PRs, gana el de número
// más alto: es el intento vigente. Quedarse con el primero mostraría un PR viejo y cerrado como si
// fuera el estado de hoy.
func indexPRs(rawItems []rawPR) map[string]*PullRequest {
	out := map[string]*PullRequest{}
	for _, p := range rawItems {
		if prev, ok := out[p.HeadRefName]; ok && prev.Number > p.Number {
			continue
		}
		out[p.HeadRefName] = &PullRequest{
			Number: p.Number, State: p.State, Base: p.BaseRefName, URL: p.URL,
			Revision: p.ReviewDecision, Draft: p.IsDraft, Merged: p.MergedAt,
