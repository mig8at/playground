...
// lastTouches: para cada archivo de tarea, la fecha (YYYY-MM-DD) del último commit que lo tocó, o
...
func lastTouches(dir string) map[string]string {
	...
	for _, l := range strings.Split(git("status", "--porcelain", "--", "."), "\n") {
		if len(l) > 3 {
			out[filepath.Base(strings.TrimSpace(l[3:]))] = today
		}
	}
	date := ""
