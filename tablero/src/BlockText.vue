...
import { inlineParts, repoHref } from './block-body.js';
...
<template>
  <template v-for="(part, index) in parts" :key="index"><code v-if="part.type === 'code'" class="inline-code">{{ part.value }}</code><strong v-else-if="part.type === 'strong'">{{ part.value }}</strong><button v-else-if="part.type === 'link' && part.kind === 'block'" type="button" class="ref ref-block" @click="emit('block', part.id)">{{ part.label }}</button><component :is="hrefOf(part) ? 'a' : 'span'" v-else-if="part.type === 'link'" class="ref" :class="'ref-' + part.kind" :href="hrefOf(part) || undefined" :title="titleOf(part)" :target="hrefOf(part) ? '_blank' : undefined" :rel="hrefOf(part) ? 'noopener' : undefined"><span v-if="part.kind === 'repo' || part.kind === 'pr'" class="ref-source">{{ part.repo }} · </span>{{ part.label }}</component><template v-else>{{ part.value }}</template></template>
</template>
