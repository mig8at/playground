...
const DB_NAME = 'trazador-consultas'
const VERSION_BD = 2
...
function searchAlias(q, results) {
  ...
  if (people.length === 1) {
    for (const value of [people[0]?.documento, people[0]?.telefono]) {
...
function openDB() {
  ...
  return new Promise((resolver) => {
    let loanRequest
    try { loanRequest = indexedDB.open(DB_NAME, VERSION_BD) }
    catch { resolver(null); return }
    loanRequest.onupgradeneeded = () => {
      const trace = loanRequest.result.objectStoreNames.contains(TRACE_STORE)
        ? loanRequest.transaction.objectStore(TRACE_STORE)
        : loanRequest.result.createObjectStore(TRACE_STORE, { keyPath: 'id' })
      if (!trace.indexNames.contains('guardadaEn')) trace.createIndex('guardadaEn', 'guardadaEn')
      const search = loanRequest.result.objectStoreNames.contains(SEARCH_STORE)
        ? loanRequest.transaction.objectStore(SEARCH_STORE)
        : loanRequest.result.createObjectStore(SEARCH_STORE, { keyPath: 'id' })
      if (!search.indexNames.contains('guardadaEn')) search.createIndex('guardadaEn', 'guardadaEn')
    }
    loanRequest.onsuccess = () => resolver(loanRequest.result)
    loanRequest.onerror = loanRequest.onblocked = () => resolver(null)
  })
}
...
export async function saveQuery({ target, traza: trace, resultados: results }) {
  try {
    ...
    await new Promise((resolver) => {
      ...
      store.put({
        id: key(target, trace.ureq), target, ureq: String(trace.ureq), guardadaEn: Date.now(), traza: trace, resultados: results,
      })
...
export async function readQuery(target, ureq) {
  try {
    ...
    return await new Promise((resolver) => {
      ...
      const loanRequest = tx.objectStore(TRACE_STORE).get(key(target, ureq))
      loanRequest.onsuccess = () => resolver(loanRequest.result || null)
      loanRequest.onerror = () => resolver(null)
...
export async function saveSearch({ target, q, resultados: results }) {
  try {
    if (!q || !results) return
    const bd = await openDB()
    if (!bd) return
    await new Promise((resolver) => {
      ...
      for (const alias of searchAlias(q, results)) {
        store.put({
          id: searchKey(target, alias), target, q: alias, consultaOriginal: String(q), guardadaEn: savedAt, resultados: results,
        })
      }
...
export async function readSearch(target, q) {
  try {
    ...
    return await new Promise((resolver) => {
      ...
      const loanRequest = tx.objectStore(SEARCH_STORE).get(searchKey(target, q))
      loanRequest.onsuccess = () => resolver(loanRequest.result || null)
      loanRequest.onerror = () => resolver(null)
      tx.oncomplete = tx.onabort = tx.onerror = () => bd.close()
    })
