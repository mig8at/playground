...
// y un árbol raíz(creditop)→flujo(motai)→tarea(motai-v2) en combinations.json.
func setupScaffoldEngine(t *testing.T) *Engine {
	...
	cf := combFile{Combinations: []Combination{
		...
		{ID: "motai", Name: "Motai", Parent: "creditop", Created: now, Updated: now},
		{ID: "motai-v2", Name: "Motai v2", Parent: "motai", Created: now, Updated: now},
	}}
...
func TestScaffoldFlowDocByRole(t *testing.T) {
	...
	cases := []struct {
		...
		{"motai", "Motai", "FLUJOMARK"},       // hijo de raíz → flujo
		{"motai-v2", "Motai v2", "TAREAMARK"}, // nieto → tarea
	}
	for _, c := range cases {
...
func TestScaffoldDoesNotClobber(t *testing.T) {
	e := setupScaffoldEngine(t)
	dir := filepath.Join(e.flowsDir, "motai")
	if err := os.MkdirAll(dir, 0o755); err != nil {
		t.Fatal(err)
	}
	existing := "# Motai · flujo\nCONTENIDO CURADO A MANO\n"
	if err := os.WriteFile(filepath.Join(dir, "map.json"), []byte(`{"name":"Motai","files":["legacy-backend/x.php"]}`), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(dir, "doc.md"), []byte(existing), 0o644); err != nil {
		t.Fatal(err)
	}
	e.ScaffoldFlowDoc("motai", "Motai") // debe ser NO-OP (ya existe map.json)
