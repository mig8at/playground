...
## Antes de concluir
- **El `initial_fee_percentage` del tramo NO está muerto en la tabla, está desconectado en v2.** El nodo padre (**CreditopX**) y **Profiling** dicen "código muerto, nunca se lee": eso es cierto para *todo el backend* y para el wizard actual, pero **falso para el front v1** — `calculate-loan-financials.uc.ts:193-194` y `application/resources/js/pages/customer/lenders/list/v2/ListLenders.vue:3317-3322` lo aplican y **pisan** el `min_initial_fee` de la categoría. Sembrar un tramo con `initial_fee_percentage > 0` cambia el enganche en la ruta v1 (application, hoy default del parallel-run). En v2 la clave top-level ni se envía → queda inerte. **Riesgo de cutover**: si alguien "arregla" v2 devolviendo `amountConditions`, reactiva una regla de enganche dormida.
...
- **Defaults 0 + falta de clamp.** Toda la tabla defaultea a 0. Con `max_amount = 0`, `CreditopXQuotaController:721-723` clampea (`max(0, …)`; re-verificado el 2026-09-19, se había corrido 253 líneas) pero los dos `LenderRetrievalService` v1 **no**: `min(max_amount, 0 − 1)` deja el cupo en **−1**.
- **Cuatro montos distintos para matchear la misma banda.** front-listado: `requestedAmount − initialFee` (`useInstallmentOptions.ts` (el monto del listado)) · front-enganche: `requestedAmount` bruto (`calculate-loan-financials:148`) · legacy plan de pagos: `userRequest->amount` crudo · application plan de pagos: `amount − administrativeCosts`. En bordes de banda el plazo que ofrece el listado puede **no coincidir** con el que habilita el plan de pagos.
- **`mandatory_fee_number` diverge front↔back.** Legacy devuelve `[max_fee_number]` aunque no esté en `fee_numbers`; el front solo puede *filtrar* lo que ya está en `installmentOptions` y, si el filtro queda vacío, **devuelve la lista completa sin filtrar** (`useInstallmentOptions.ts:70-72`) — o sea que un plazo obligatorio ausente de `fee_numbers` produce, en el front, **cero restricción**.
- **`mandatory_fee_number` es `boolean` sin `$casts`.** El front tipa `number` y compara `=== 1` (`useInstallmentOptions.ts:66`); hoy funciona porque el tinyint se serializa 0/1. Agregar un cast `'boolean'` al Model rompería silenciosamente ese `===`. El Vue usa truthiness y no se vería afectado.
- **Bandas solapadas: 3 desempates distintos.** El repo no garantiza unicidad. ⚠ **Pero el comentario que este nodo citaba como prueba —«this shouldn't happen» en `RegularPaymentScheduleService.php:156`— ya no existe**: re-verificado el 2026-09-19, esa frase no aparece en ningún archivo de `Modules/` ni de `app/`. Los tres desempates siguen siendo reales; lo que se perdió es el reconocimiento explícito del autor.
  ✔ **Y en esa misma línea hay ahora otra cosa, que le importa a este nodo más que el comentario viejo: un QUINTO monto.** El docblock de `:150-158` documenta, medido sobre la solicitud **552099**, que `amount` quedaba **congelado en la foto del marketplace** —calculado con el plazo máximo de la línea, porque el cliente todavía no había elegido— mientras `final_amount` salía correcto con el plazo real. Y dice textualmente que **la banda de `creditop_x_conditions_by_amount_by_lender` se elige con ese `amount`**. O sea: a los cuatro montos que este nodo enumera hay que sumarles el caso en que el monto que matchea la banda **es el de una elección que el cliente todavía no había hecho**.
...
- **Fuga de variable en v1.** En `processRevolvingAndCreditopXLenders` (los dos repos) `$amountConditions` se pisa en cada iteración del loop de lenders rt=2, así que el top-level que sale al front es el del **último** lender procesado. ⚠ **Sigue vivo el 2026-09-19**, y conviene no confundirse con lo que sí cambió: el bloque **ahora está memoizado por lender** (`amount_conditions_{lender_id}`, cache de array a 30 s), lo que ahorra la consulta pero **no evita el pisado** — la variable se reasigna igual. El front se protege filtrando por `r.lender_id === lender.id` — pero si un día se ordena distinto y hay 2 rt=2 con tramos, el front recibe los de uno solo.
...
**(2026-09-19) Nodo RE-VERIFICADO entero.** 13 afirmaciones auditadas —9 de código contra `main` y 4
de dato medidas contra producción—, cero chequeos débiles. ✔ **La migración sigue siendo byte a byte
idéntica en los dos repos** (mismo md5), y `max_fee_number` sigue siendo `integer` sin `nullable`, así
que la rama `=== null` sigue muerta. ⚠ **Cuánto de todo esto está vivo: poco.** Medido en prod el
2026-09-19, la tabla tiene **9 tramos en 3 entidades**. De esos: **5 traen `initial_fee_percentage > 0`**
—o sea que el dato del primer bullet **existe y está poblado**, sólo que v2 no lo consume— y **4 fuerzan
plazo único**. ✔ Y el peligro que el nodo advierte sobre la falta de clamp **no está materializado**:
**ninguna fila tiene `max_amount = 0`**. Lo corregido: el clamp se había corrido 253 líneas, y el
comentario que sostenía el bullet de bandas solapadas desapareció — pero en su lugar apareció un quinto
monto que el nodo no contaba.
...
- front: `useInstallmentOptions.ts:39-75` filtra `installmentOptions` con `max_fee_number` y, si `mandatory_fee_number === 1`, deja solo esa opción — matcheando contra el **monto financiado** (`requestedAmount − initialFee`, `:108`), no el bruto.

⚠ **Dos cambios en el cálculo del enganche (2026-07-21), en el mismo archivo:**
- **El enganche mínimo ahora es `max(excedente, política)`** — `calculate-loan-financials.uc.ts:175-177`.
  Antes, si el valor con costos administrativos superaba el cupo, devolvía **solo** el excedente y el
  `initial_fee_percentage` de política quedaba ignorado; hoy toma el mayor de los dos, así que nunca
  queda por debajo de la política aunque el excedente sea menor.
- **Los costos administrativos salen SIEMPRE del lender** (`:70`), ya no de
  `externalFinancials.comisionAliado`. Para Welli Riesgo Compartido ese valor **ya es** el
  `comision_aliado` (lo setea `applyWelliSharedRiskOverrides` desde el pre-approval) y el
  `update-amount` no reenvía la comisión, así que pisarlo lo borraba.

**3 · Fija el enganche — solo en la ruta v1.**
`initial_fee_percentage` del tramo **pisa** el del lender (que en v1 el backend ya había sobreescrito con `category->min_initial_fee`): `calculate-loan-financials.uc.ts:180-198` (React) y `application/resources/js/pages/customer/lenders/list/v2/ListLenders.vue:3317-3322` (Vue de application), misma lógica: buscar la banda por `lender_id` + monto, y si `rule.initial_fee_percentage != null` devolverla. **Ningún backend lee esta columna** — es 100% decisión de cliente.
⚠ En el wizard actual **nunca se ejerce**: el wizard pega a `lenders-v2` (`loan-options.repository.ts:27`) y **v2 no devuelve la clave top-level `amountConditions`** (`processRevolvingAndCreditopXLenders` está comentado, `LenderListingService.php:178-182`) → `data.data.amountConditions ?? null` (`:67`) queda **null** → `getInitialFeePercentage` cae siempre al `lender.initial_fee_percentage`. Ver Gotchas.
...
### Cableado (legacy)
`CreditopXConditionsByAmountByLenderRepositoryInterface` (2 métodos) → `…Repository`, bindeado en `LoansServiceProvider.php:327-330`. `getByLenderAndAmount` = `min_amount <= $amount AND max_amount > $amount`; `getAllByLenderIds` = `whereIn` + `groupBy('lender_id')` (devuelve `collect()` vacío si el array viene vacío). Consumidores inyectados: `CreditopXQuotaController`, `RegularPaymentScheduleService`, `LenderListingService`. **application no tiene repositorio**: usa el Model directo con `Cache::store('array')->remember(…, 30)` (cache en memoria del request, `application/app/Services/lenders/LenderRetrievalService.php:700-703`).
...
- **Tope de cupo — listado v1**: application `app/Services/lenders/LenderRetrievalService.php:700-703` (cache) · `:738-743` (filtro por plazo de categoría) · `:745-760` (tope `−1`, **sin clamp a 0**) · `:267`/`:324`/`:823` (payload `amountConditions`); legacy `Modules/Onboarding/App/Services/lenders/LenderRetrievalService.php:831-834` · `:787-810` (idem + `:806` pega `amount_conditions`) · `:290`/`:361`/`:895`.
...
- **Front React** (`frontend-monorepo/modules/loan-request-wizard/lenders-marketplace/src/`): `lib/application/calculate-loan-financials.uc.ts:142-174` (enganche mínimo / excedente) · `:176-194` (enganche del tramo) · `components/available-lenders/hooks/useInstallmentOptions.ts:21-73` (tipo + filtro de plazos) · `:108` (matchea con el monto financiado) · `lib/types/lender-api-response.ts:52-58` y `lib/domain/entities/loan-option.entity.ts:160-170` (los **dos** shapes: top-level vs per-lender) · `lib/infrastructure/repositories/loan-options.repository.ts:27` (v2) `:67` (`amountConditions ?? null`) · `lib/mappers/lender-response.mapper.ts:275` · `frontend-monorepo/modules/loan-request-wizard/lenders-marketplace/src/components/available-lenders/AvailableLenders.tsx:77,121,328` (⚠ acá se citaba `components/available-lenders/AvailableLenders.tsx:77,121,328` (⚠ acá se citaba el *LenderMarketplaceContext* en su línea 378; ese archivo tiene **376 líneas** y ni una mención de tramos — la lógica se mudó al componente y a `ExternalAmountUpdater`/`MeddipayAmountRepricer`), y ese archivo tiene **376 líneas** y ni una mención de tramos: la lógica se mudó al componente y a `ExternalAmountUpdater`/`MeddipayAmountRepricer`) (`useAmountConditions`) · `components/lender-card/useLenderFinancials.ts:32` · `components/available-lenders/hooks/useInitialFee.ts:48-71`; inyección desde la ruta: `apps/loan-request-wizard/app/routes/lenders-marketplace/available-lenders.tsx:181`.
