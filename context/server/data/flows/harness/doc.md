...
- **Muro Wompi (cierre rt=2 por UI NO headless)**: no es falta de testids. #77 exige cuota inicial → redirige a **Wompi hosted checkout** (externo; `WOMPI_MOCK` aplica al webhook, no al checkout); #37 responde `/continue?url=null` → **404**. El cierre rt=2 → Estado 11 **ya está validado en backend** (`asesor 3e67eade 77` fuerza `initial_fee=0`). `closeCreditopX` **lanza** con ese diagnóstico a propósito.
