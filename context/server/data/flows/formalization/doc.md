...
Redis** y `OtpService` lo lee con reintentos; Al depurar un OTP que no llega, el sospechoso ya no es Twilio sino el MS y Redis.

⚠ **Acá decía que «solo en `local`, si el cache no entrega, cae a un OTP fijo». Ese mecanismo ya no existe.** Verificado contra `main` el 2026-09-18: lo que hay es un **bypass por TELÉFONO**, no por ambiente-más-fallo-de-cache. Si el número está en la lista del setting `qa_otp_bypass_phones`, el OTP son los **últimos 6 dígitos del celular normalizado**, **no se manda SMS ni WhatsApp**, y —esto es lo que lo hace utilizable— **el registro del OTP SÍ se persiste encriptado**, así que `validateOtp` lo encuentra por la vía normal y el código viaja hasta la firma del documento (`legacy-backend/Modules/Loans/App/Services/OtpService.php:48-52`, `:212`). ✔ **Está atado al ambiente y la guarda es explícita:** sólo `local` o `development`; cualquier otro —production, staging, testing— **ignora la lista por completo**, y cada uso deja un log de nivel `warning` (`legacy-backend/Modules/Onboarding/App/Services/OtpBypassService.php:14-18`). ⚠ Ojo con staging: su `APP_ENV` efectivo es `develop`, que **no** es `development` — o sea que por código el bypass ahí está APAGADO. Ver la disputa en el `CLAUDE.md` raíz antes de armar una prueba encima.
...
- **Bug real: argumento descartado en el envío de OTP.** `ValidateOtpPromissoryNoteController.php:229-234` llama `sendOtpPromissoryNote($user, $action, $phoneCode, $isImei ? 'service' : null)` con **4 argumentos** contra una firma de **3** (`legacy-backend/Modules/Loans/App/Services/OtpService.php:44`). PHP descarta el extra en silencio → la selección de canal para IMEI nunca llega al servicio. **Verificado contra `main` el 2026-09-18: sigue vivo**, y el servicio ya no vive donde decía este nodo — es `Modules/Loans`, no `Modules/Onboarding`.
...
1. **`confirmation`** → `POST /api/loans/customer/requests/confirm` (`ContinueUserFlowController::confirm:68`). El `step_details.type` enruta a validación de identidad (`aws_ / ado_ / crosscore_ / evidente_validation`) o directo a `first-payment-date` si `no_validation_required` (`loan-confirmation.tsx:243-265`). → nodo **kyc**.
   <br>**Quién decide cuál**: el LENDER. `CreditopXFlowService.php:117` lee `lender_identity_validation_types.identity_validation_type_id` con fallback a `lenders.validation_type`, y `IdentityValidationStepResolver::resolve` traduce ese id al `next_step` + `step_details` — es la fuente única del mapeo (antes estaba duplicado dentro de `CreditopXFlowService`). Enum de 7 valores en `Modules/Identity/App/Enums/IdentityValidationType.php`: `0 Unknown · 1 None · 2 AwsOcrRekognition · 3 Questions · 4 Ado · 5 CrossCore · 6 Evidente`.
   <br>⚠ **Este paso es CONDICIONAL y hay tres formas de que no se vea**: `1 None` lo salta entero (9 lenders in-platform en prod), una validación manual reciente devuelve también `no_validation_required`, y **`2 AwsOcrRekognition` corre completo sin escribir una sola fila en `risk_central_user_data`** (46 de los 119 lenders in-platform — el segundo grupo más grande después de Ado con 64). Medido en prod el 2026-08-05. Al auditar este tramo por BD, para casi la mitad de los lenders no hay nada que encontrar y eso **no** es una falla: el rastro está en los logs de `Modules/Identity`. Detalle y conteos → nodo **kyc** (F-101).
...
En el ledger, `createFirstRegister` marca el primer registro con `status = 8` en vez de 1 cuando `lender.externally_serviced` es true, para excluirlo de la gestión diaria de cartera (`CreditopXRequestHistoryService.php:373-377`).
...
### Un voucher generado a mano ahora se distingue del automático, en el propio PDF

Desde el 2026-08-06 el voucher producido **desde el panel** imprime `** GENERADO MANUALMENTE **` debajo del pie `** CLIENTE **`, así que un documento que **no** salió del flujo automático de desembolso se puede distinguir después **sin ir a los logs**. `generateVoucher` recibe un tercer argumento opcional que por defecto es falso, de modo que los otros **13 puntos de llamada** se comportan exactamente igual y **sólo el botón del admin** lo pasa; la vista se protege con `$manual ?? false` para los llamadores que la renderizan sin la variable.

⚠ **Y un detalle que conviene saber si se toca esa vista: la variable `user` que el controlador le pasa está MUERTA** — el pie está **quemado** como `** CLIENTE **` en la plantilla y nunca la lee. Quien intente personalizar el pie desde el controlador no va a ver ningún efecto.
...
- **Endpoints de firma** (legacy-backend): `Modules/Loans/App/Http/Controllers/Customer/ValidateOtpPromissoryNoteController.php:200` sendOtp · `:321` verifyOtp · `:364` disburse · `:449` notifyEcommerceStore (privado, llamado desde `:399`). *(Los cuatro se corrieron entre 50 y 70 líneas; revalidado contra `main` el 2026-09-18.)*
...
- **Handoff desde la selección** (legacy-backend): `Modules/Onboarding/App/Services/UserRequestService.php:294` (estado 3), `Modules/Onboarding/App/Services/UserRequestService.php:489-505` y `Modules/Onboarding/App/Services/UserRequestService.php:674-683` (`standBy` + URL `/self-service/.../confirmation`), `Modules/Onboarding/App/Services/UserRequestService.php:513-673` (radicación rt=1).
