...
## Bitácora
- **2026-07-29** — **Nodo reescrito a la v2.** Se fue todo lo de modos (tabla de 3 modos, bypass `isMotaiRenting`, `partner_modes`, `MOTAI_RENTING_ALLIED_MODE_ID`, el filtro NO-OP por modo) y entró el modelo config-driven: `lender_requirements` como fuente única de Ábaco, `next_step` dirigiendo el flujo, la calculadora en backend (`lenders.calculator` + `FormulaCalculator`) y los reintentos de Ábaco. Verificado contra `origin/qa` archivo por archivo (`MotaiValidationService` ya no existe; `MOTAI_LENDER_IDS`, `getMotaiTotalAmount` y `merchant-mode.tsx` borrados del front; la decisión manual del asesor y `average_income` huérfano SÍ siguen). Se preservó íntegra la sección legal/calculadora del 2026-07-28.
- **2026-07-29** — **MERGEADO a `qa`** (PR #1020 des-motaización + #1028 Ábaco) y **validado contra el ambiente de qa**: el 158 lista · `MOTV1001` resuelto por la tabla · cadena gig corriendo · estado de reintentos persistido. Muros del camino: `findings` **F-73** (el backend de qa es otro servicio del mismo cluster), **F-74** (el sweep escribía en dos bases), **F-75** (marketplace vacío por `group_rules`).
- **2026-07-28** — Documentada la **razón legal** de las dos líneas (explicada por Manuela): sin opción de compra no hay interés → no es crédito → **no aplica el techo de usura**; con opción de compra sí. Verificado contra `motai-manu.pdf` y la calculadora: el mismo 1,8% figura como "**Parámetro**" en la pestaña Renting y como "**Equivale a ~0,4125% semanal**" en Rent to Own. Eso **cierra la pregunta abierta del +1,11%**: no era una conversión mal hecha. En `playground/engine` la constante pasó de `monthlyRate` a **`anchorRate`** y cada hoja declara `legalNature`, para que "arreglar" el prorrateo no sea un cambio silencioso de producto.
- **2026-07-24** — Validado E2E en local el trigger de Ábaco **por-producto** (el puente transitorio, hoy reemplazado por `lender_requirements`). El guiado del harness no lo mostraba por un salto directo a first-payment (arreglado: ahora recorre `/abaco` y auto-maneja las plataformas gig). Detalle en `findings` **F-68**.
- **2026-07-23** — La des-motaización quedó reasentada limpia sobre `qa` en `feature/motai-clean-v2` (legacy-backend + frontend-monorepo).
- **2026-07-18** — RENOMBRADO `motaix` → `motai` (nodo + id + refs). El nodo cuelga de **Merchants**, así que documenta el **COMERCIO** aliado 158 (Motai); **MotaiX** es su *lender* rt=2 y pertenece conceptualmente a **Entities**.
- **2026-07-17** — Fase de data: superficie de código curada + doc enriquecido desde `git 159906a:docs/codigo/MOTAI-FLUJO-ANALISIS.md` y `…/mejoras/MOTAI-PLAN-EVOLUCION.md`; líneas re-verificadas contra el código real.
