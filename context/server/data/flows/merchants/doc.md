...
- **`have_ctopx` es NO-OP en el listado de legacy.** `LenderListingService.php:356` tiene `$no_more = false; // TODO quitar definitivamente`; con eso, `have_ctopx = false` deja la lista vacía y el `elseif (empty(...))` de `Modules/Onboarding/App/Services/lenders/LenderListingService.php:377` la rellena con **todas** las entidades de la sucursal. Ambos valores del flag terminan igual. En `application` (`LenderRetrievalService.php:126`) `$no_more` **sí** se calcula (¿ya tiene un crédito rt=2 en estado 11 en ese comercio?) → **la regla "un CreditopX activo por comercio" existe en application y se perdió en legacy**.
...
1. `app/Http/Controllers/Admin/AlliedAlliedBranchController.php:123` — **borra todas** las filas de `lenders_by_allied_branches` de esa sucursal.
2. `app/Http/Controllers/Admin/AlliedAlliedBranchController.php:130` — las **recrea** desde el payload (`url_utm` se guarda `NULL` si coincide con el del comercio → herencia por `COALESCE` al leer, `app/Http/Controllers/Admin/AlliedAlliedBranchController.php:77`).
...
- `routes/admin.php:42-71` el mapa de pantallas (`aliados`, `aliados.puntosdeventa` `routes/admin.php:58`, `editar-disparador` `routes/admin.php:59`, `aliados.entidades` `routes/admin.php:66`, `aliados.modulos` `routes/admin.php:70`, `aliados.ecommerce` `routes/admin.php:71`) · `routes/admin.php:145-149` reglas.
