...
- **NO valida semántica al guardar** (`validators.go`): chequea la FORMA, no el significado. NO chequea que el `field_id` pertenezca al `form_type`, ni tipo, ni requeridos → **el schema se autoría en el backend pero se enforcea en el front**; un cliente roto escribe basura tipada como string y los document builders la leen por magic-number.
- **Qué acepta hoy, exactamente** (`validAnswerValue`, verificado contra `main` el 2026-09-18): `null`; **cualquier string, incluidos el vacío y el de sólo espacios** —los dos codifican «el usuario dejó esto en blanco», que es legítimo para los campos opcionales, y el mapper los convierte en **`NULL` de SQL**—; y **números enteros O decimales**, rechazando sólo los literales fuera del rango de `float64`. ⚠ **El número se persiste como su literal original, nunca como un `float` parseado**, así que `54.11` llega textual a la columna y no se pierde precisión. **Booleanos, arrays y objetos siguen rechazados**, porque la columna legacy no puede representarlos: **el multiselect no pasa**.
  *(Acá decía «string NO vacío o entero» y que «decimales / multiselect NO pasan». Lo de los decimales dejó de ser cierto y este mismo nodo ya lo decía más abajo desde el 2026-08-28: el documento se contradecía a sí mismo, con la mitad vieja justo en el bloque que se lee primero. El string vacío nunca estuvo prohibido. ⚠ Y el docblock del propio `validators.go:45-47` **también quedó viejo** —sigue diciendo «integer numbers»—, así que ahí tampoco conviene creerle al comentario.)*
- **La rama JSON del mapper NO es código muerto** —acá se decía que sí—: `responseValueString` devuelve `NULL` para `nil` y para strings en blanco, deja pasar los strings con contenido tal cual, y **delega todo lo demás a `ToJson`**. Como el validador ya filtró booleanos y compuestos, **por esa rama pasa hoy TODO número**, incluidos los decimales. Lo que sí es inalcanzable es su capacidad de serializar booleanos y compuestos.
...
**(2026-09-18) Nodo RE-VERIFICADO entero.** La deriva no podía encontrar lo que apareció: **el documento
se contradecía a sí mismo**. «Antes de concluir» afirmaba que los decimales no pasan y una nota de
2026-08-28, sesenta líneas más abajo, decía lo contrario — y la versión vieja estaba **en el bloque que
un modelo lee primero**. Verificado contra `main`: pasan los decimales, pasa el string vacío (mapeado a
`NULL`), y la rama JSON del mapper que se daba por muerta es el camino de **todos** los números.
Ningún instrumento de deriva iba a llegar ahí, porque el archivo se había movido y alguien ya lo había
leído: lo que faltaba era **releer el nodo contra sí mismo**.
