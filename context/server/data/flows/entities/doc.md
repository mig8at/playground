...
- **Código muerto en el gemelo legacy**: `LenderRepository::getActive()` hace `Lender::where('status', 'Activo')` sobre una columna **booleana** (nunca matchea) y `getPaginated()` filtra por `$filters['type']`, columna que **no existe** en `lenders`. Ambos métodos están en la interfaz (`Modules/Partner/Contracts/Repositories/LenderRepositoryInterface.php:19-20`) y **no los llama nadie**.
