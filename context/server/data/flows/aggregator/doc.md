...
- **Selección + canal** (legacy-backend): `Modules/Onboarding/App/Services/UserRequestService.php:444` `case 1` → `:502` `$lender->action` (`:541-543` variantes Welli, `:597`/`:615` Meddipay `openProcessModal`, `:643` Lagobo y `:653` Davivienda = modal manual sin url) · `LenderTabBehaviorResolver.php:22` `NON_NEW_TAB_LENDER_NAMES`.
...
- Subcontexto: **bancolombia** (BNPL 68 / Consumo 100 — la máquina de originación multi-step que corre DENTRO de CreditOp: onboarding propio, 23 endpoints, JWT RS256 + mTLS, sandbox por cédula/celular, código de compra en punto de venta, y el webhook que sigue en application).
- Cruce (**ya NO es subcontexto**, se re-parentó a `merchants` el 2026-07-31): **corbeta** (retail físico Corbeta/allied 24 — Alkosto/K-Tronix/Alkomprar; checkout ecommerce base64 → PIN de la API Fondos → factura en caja → conciliación batch → **estado 26 Facturado** → confirmación diferida a Bancolombia BNPL/Consumo). Es un **grupo de comercios**, no una familia de prestamistas: por eso vive en `merchants`. Ojo con el número: Bancolombia **no** es "el lender de Corbeta" — 68 está habilitado en **109 de 230** comercios y el 100 en **78**; lo que sí es cierto al revés es que los tres retail Corbeta tienen **sólo** 68 y 100. Este nodo conserva la maquinaria genérica rt=1. Pre-aprobación v2 (Capa 1b): **ms-preapprovals** (MS Go, workflow de 4 etapas + adapters + espejo a legacy). Fuente profunda: `git 159906a:docs/codigo/AGREGADORES-FLUJO-ANALISIS.md`.
