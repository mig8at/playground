...
- **`UserRequest::lenderIntegrationFlow()`** (`legacy-backend/app/Models/UserRequest.php:234`; acá decía `:207`, y ⚠ **el método NO existe en `application`** — buscarlo por nombre sin repo no encuentra nada ahí) es un `hasOne` **sin filtro de lender**, y la tabla mezcla lenders (local: 100→1.730, 68→223, 24→8, 23→1, 6→1). No hay índice único sobre `user_request_id`: **16** `user_request_id` con más de una fila en la copia local. Usar siempre `getStepsFromSession`.
...
chequeos débiles y ninguna falsa. Es el nodo con **mejor higiene de citas del árbol**: 81 de 86
referencias ancladas, una sola corrida. Exactos: `Bancolombia::register()` **sigue literalmente vacío**,
el `getRequestExceptionCode()` que indexa `['errors'][0]['code']` sin guarda, y el `switch` de Corbeta
con sus cuatro allieds (24 · 209 Alkosto · 210 K-TRONIX · 211 Alkomprar) conviviendo con el Setting
dinámico. Corregidas **dos citas**, y las dos por la misma razón: se escribieron **sin repo** y el
archivo existe en los dos monolitos —`lenderIntegrationFlow()` ni siquiera existe en `application`—.
Además la del `bnpl_transaction_id` apuntaba a una **lectura** del valor y no a su escritura, que vive
en el controlador.
...
### 3 · BNPL 68 — la secuencia real (prefijo `bancolombia-bnpl/`, `Modules/Onboarding/routes/api.php:83-92`)
