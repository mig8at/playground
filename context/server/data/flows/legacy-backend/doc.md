...
**(2026-09-19) Nodo RE-VERIFICADO entero.** 14 afirmaciones auditadas —12 de código contra `main` y 2
de dato—, cero chequeos débiles. **Una resultó ARREGLADA**: el `phpunit.xml` ya no tiene el desajuste
de mayúscula que impedía correr la suite de módulos. ✔ Lo demás se sostuvo exacto, incluido lo más
específico: `Modules/Risk/routes/api.php` sigue **sin declarar una sola ruta** con sus 16 controladores,
`routes/api.php` de Loans se sigue cargando **exactamente dos veces** con prefijos y namespaces
distintos, el «archivo de rutas» que en realidad es una migración **sigue ahí**, y `loadMigrationsFrom`
sigue apuntando a una carpeta `Database/migrations` **que no existe** (sólo hay `Database/Seeders`). Y
el dato que más cambió no es un error del nodo sino una tendencia: las 286 migraciones compartidas
**siguen siendo 286**, pero las exclusivas de este repo pasaron de 67 a **171**.

## Antes de concluir
- **En Risk, los "controladores" son servicios.** `Modules/Risk/routes/api.php` no declara ni una ruta, pero sus 16 controladores (media 618 líneas) se instancian a mano desde Onboarding, Partner y `app/Actions`: `ListLenderController.php:98` hace `app(\Modules\Risk\…\ProfilingReviewController::class)->updateAsyncLender(...)`. Sus métodos no reciben `Request` ni devuelven `JsonResponse` — reciben modelos y devuelven arrays (`ProfilingReviewController.php:278,358`; `DatacreditoQueryByAlliedController.php:38`). El anti-patrón espejo también existe: **6 servicios de Onboarding heredan `ApiController`** (p.ej. `OnboardingService.php:38`) solo para usar `success()/error()`.
...
- ✅ **El fork de `LenderUserCategoryService` SE RESOLVIÓ — este bullet describía dos servicios y hoy queda uno.** Decía que convivían `Loans/App/Services/LenderUserCategoryService.php` (742 líneas, tipado) y `Onboarding/App/Services/lenders/LenderUserCategoryService.php` (417 líneas, sin tipos), y que `LenderListingService` **inyectaba los dos a la vez**. Verificado contra `main` el 2026-09-19: **la copia de Onboarding ya no existe**; el único es el de Loans, que creció a **1.064 líneas**, y `LenderListingService` lo inyecta **una sola vez**, aliasado `…ServiceCtopX` (`Modules/Onboarding/App/Services/lenders/LenderListingService.php:17` el `use`, `:31` el constructor). Sigue usándolo para sellar la aprobación en `:293` (`stampCreditopXApproval`, definido en `:550`). Es el mismo retiro que registra `hardcodes-entidades` para el motor de categorías: **quedó uno solo, en Loans**.
- **`LenderListingService extends LenderRetrievalService`** (`:23`): el listado v2 es una **subclase** del v1, no un reemplazo. Y su `stampCreditopXApproval` documenta (`:279`) que NO corre los gates de crédito activo / reglas de lender / datacrédito que sí aplica `/available-quota` → el sello del listado y el cupo pueden discrepar.
...
- ✅ **La suite `Modules` de PHPUnit YA matchea — este bullet describía un bug que se arregló.** Decía que `phpunit.xml` declaraba `./Modules/*/Tests/{Feature,Unit}` con **T mayúscula** mientras los directorios versionados son minúscula, así que en Linux no corría ni un test. Verificado contra `main` el 2026-09-19: **el `phpunit.xml` dice hoy `./Modules/*/tests/Feature` y `./Modules/*/tests/Unit`, en minúscula** (`:15-16`), y los directorios siguen siendo `Modules/<X>/tests/`. Coinciden. ⚠ **Y eso cambia el riesgo que describe el `CLAUDE.md` raíz**: la suite de módulos que antes no corría, ahora corre — incluido el `Modules/Backoffice/Tests/Feature/LenderRulesWriterServiceTest.php` que lleva `RefreshDatabase`.
...
- **Riesgo de esquema compartido**: **286** migraciones con nombre idéntico en ambos repos —re-contadas el 2026-09-19 y **el número no se movió**— más **171** exclusivas de legacy y **51** de application. ⚠ **Ese contraste es el dato**: el conjunto compartido está **congelado** mientras el exclusivo de legacy **casi se triplicó** (eran 67). La copia a mano dejó de hacerse y la divergencia corre en un solo sentido (ver `architecture`), todas sobre la misma tabla `migrations` de la misma BD. Cada app aplica su propio set.
- **Hardcodes**: `config/lenders.php:24` resuelve `smartpay_lender_id` por `env('APP_ENV')` (160 en producción, 153 en el resto) — no por BD ni por env propia. `LenderListingController.php:22` usa `amount` default `180000`.
- **Fuga de frontera módulo→raíz**: `Modules/Onboarding/routes/api.php:112-153` y `routes/webhooks.php:36` rutean a controladores de `app/Http/Controllers/Api/CredifamiliaV2/`, fuera de todo módulo.
...
⚠ **`config/app.php` trae `backoffice_host` con un default LOCAL.** `'backoffice_host' => env('BACKOFFICE_HOST', 'http://localhost:5175')` — es la base del **nuevo panel Backoffice (React)**, separada a propósito de `ADMIN_URL` (el admin viejo), y con ella se arma el link de validación manual que se le manda a usuarios internos. El propio código avisa: si `BACKOFFICE_HOST` no está seteado en el ambiente, **esos links salen apuntando a `localhost:5175`**.
...
- **25 alias de middleware** en `app/Http/Kernel.php:49-73`; dos vienen de módulos (`backdoor.api_key` de Onboarding `:70`, `origination.flow` de Loans `:73`).
...
| `GET /api/onboarding/loan-application/lenders/{id}` | `application/app/Http/Controllers/Customer/ListLenderController.php:252-262` | **COMENTADO** (delegación del listado desactivada) |
...
**Nueva arquitectura**:
- `Modules/CommonsV1/App/Services/BaseService.php:77,70,101` (envelope + `runServiceMethod`) · `App/Utils/TracingUtils.php:37` · `App/Services/CacheService.php:27` · `App/Providers/CommonsV1ServiceProvider.php:19-31` (bindings + singletons)
...
- `Modules/OnboardingV2/routes/api.php:24,35` · `App/Providers/RouteServiceProvider.php:24` · `App/Services/ValidateOtpAuthService.php:72,153,256,414` · `App/Services/StorePersonalInfoService.php:122` (**siempre HTTP 501**)
- `Modules/RiskV2/routes/api.php:26,31` · `App/Constants/RiskCentral.php:15-21,41-48` (ids de buró + columna espejo en `user_summaries`) · `App/Extractors/RiskCentral/MareiguaExtractor.php:65` (port del legacy; se apoya en funciones SQL `FN_Mareigua_*`)
...
**Módulos legacy**:
- **Onboarding**: `routes/api.php:49` (listado v1) `:50` (lenders-v2) `:93-134` (rutea a controladores de la raíz) · `routes/webhooks.php:36,18,24,32` (callbacks del MS de lenders + VTEX, sin Cognito) · `App/Providers/RouteServiceProvider.php:41-44` · `App/Providers/OnboardingServiceProvider.php:24` (`loadMigrationsFrom` a un dir inexistente) · `App/Http/Controllers/OnboardingController.php:149,509,899` · `App/Http/Controllers/ListLenderController.php:98,139` · `App/Http/Controllers/LenderListingController.php:21` (monto default `180000`) · `App/Services/OnboardingService.php:36` · `App/Services/UserRequestService.php:72` · `App/Services/lenders/LenderListingService.php:20,29-30,285,298` · `App/Services/lenders/LenderRetrievalService.php:83` · `App/Services/lenders/RiskCentralValidationService.php:26,46,56` · `App/Services/lenders/PreApprovedLenderService.php` · *(el `LenderUserCategoryService` de Onboarding **ya no existe**: se retiró y quedó sólo el de `Loans` — ver «Antes de concluir»)*
- **Loans**: `routes/api.php:107` (`/lender/available-quota`) · `routes/admin.php` · `App/Providers/RouteServiceProvider.php:40,45,50` (**doble registro**) · `App/Providers/LoansServiceProvider.php` (86 bindings) · `App/Http/Controllers/Customer/CreditopXQuotaController.php:81` · `App/Services/DatacreditoRuleEvaluator.php:21,25,29,48,94` · `App/Services/LenderRuleEvaluator.php:19` · `App/Services/LenderUserCategoryService.php:61` · `App/Repositories/LenderDatacreditoRulesRepository.php:24-26` · `App/Http/Middleware/AddOriginationFlowType.php:27` · `routes/add_selected_payment_date_to_user_requests.php` (**migración traspapelada**)
- **Risk**: `routes/api.php` (**vacío**) · `App/Http/Controllers/Customer/ProfilingReviewController.php:287,358` · `App/Http/Controllers/DatacreditoQueryByAlliedController.php:38,240` · `App/Http/Controllers/Admin/ProfilingController.php` (4.767 líneas)
...
**Integraciones, mocks y datos**: `app/Actions/RiskCentrals/Experian.php:481-483,512` (variantes Acierta/Quanto + `ProductId 64`) · `app/Support/Onboarding/External/Http/HttpFakeRegistrar.php:25` · `app/Providers/OnboardingDriverServiceProvider.php:40,73,110,141` · `config/onboarding.php:20-33` · `config/services.php` · `config/lenders.php:25` · `app/Services/Lenders/CredifamiliaV2/Evidente/EvidenteFlowService.php` · `app/Http/Controllers/Api/CredifamiliaV2/EvidenteController.php` · `app/Console/Commands/UpdateCreditopXRequestsCommand.php` (gemelo no agendado) · `Modules/Identity/App/Models/IdentityValidationAttempt.php` (**el único modelo dentro de un módulo**) · `Modules/Loans/tests/Feature/CreditopXQuotaControllerTest.php:36` (el único ejercicio de `/available-quota`)

**Frontera parallel-run** (`application`): `application/config/services.php:224,228` · `app/Http/Controllers/Customer/ValidateOtpController.php:112,122` · `app/Http/Controllers/Customer/PersonalInfoController.php:1019,1036,1044` · `app/Http/Controllers/Customer/ListLenderController.php:252` · `application/app/Console/Kernel.php` (24 comandos agendados; el rango `:19-65` que se citaba acá ya no delimita el bloque) (~20 tareas de servicing, contra las 4 de legacy) · `routes/api.php` (la API de application son **solo webhooks** de lenders externos)
