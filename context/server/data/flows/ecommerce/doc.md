...
## Preguntas abiertas
> **Respuesta parcial (Miguel, 2026-07-18):** la reubicación del canal a legacy-backend + wizard es un desarrollo REAL pero **sin mergear** — vive en la task **ecommerce-web-origination** (PRs [503](https://github.com/Creditop-SAS/legacy-backend/pull/503) / [363](https://github.com/Creditop-SAS/frontend-monorepo/pull/363), rama `feature/onboarding/ecommerce-web-origination`, probado en dev). Por eso las rutas del front no están en `main`. Hasta que se mergee, prod sigue sirviendo el ecommerce no-Corbeta desde el monolito.
- ¿Cuándo se completa el cutover del resto de comercios ecommerce (los que no están en `[24,209,210,211,311]`) del monolito a legacy-backend? Hoy la mayoría del checkout WooCommerce/self sigue server-rendered en `application`; el trabajo que lo movería es la task de arriba, aún sin merge.
- La entrada unificada `/ecommerce/{hash}/checkout` del **front** no está en `main` (vive en la rama de la task). En prod hoy el entry lo sirve el **monolito** para no-Corbeta; el wizard nuevo (`checkout-redirection.tsx` → `ecommerce-continue.tsx`) queda activo cuando se mergee #363.
