...
- **BUG `min_income` NO-OP** (vivo): la columna del tier es `monthly_income` (`migration:21`) pero `evaluateEligibility` lee `$rule->min_income` (`Modules/Loans/App/Services/LenderUserCategoryService.php:416`) — atributo inexistente → `null` → `$salary >= null` es **siempre true** en PHP. El **piso de ingreso de la categoría no filtra**; arreglarlo (leer `monthly_income`) **endurece** la asignación. [MEMORY flow-reorg-y-mapa-atributos]
...
2. `evaluateEligibility:403` = **AND con early-exit**: ocupación (`Modules/Loans/App/Services/LenderUserCategoryService.php:407`) → edad `users.age∈[min,max]` (`Modules/Loans/App/Services/LenderUserCategoryService.php:412`) → `min_income` (`Modules/Loans/App/Services/LenderUserCategoryService.php:416`) → género (`Modules/Loans/App/Services/LenderUserCategoryService.php:419`) → continuidad (`Modules/Loans/App/Services/LenderUserCategoryService.php:421`) → **exige `$user->datacredito`** (`Modules/Loans/App/Services/LenderUserCategoryService.php:429`) → sub-checks de buró: neg12 `<=` (`Modules/Loans/App/Services/LenderUserCategoryService.php:440`), moras `currentNegativeCredits<=` (`Modules/Loans/App/Services/LenderUserCategoryService.php:448`), maturación `diffInMonths>=` (`Modules/Loans/App/Services/LenderUserCategoryService.php:453`), score `>=` (`Modules/Loans/App/Services/LenderUserCategoryService.php:459`), TC/vectores (`Modules/Loans/App/Services/LenderUserCategoryService.php:464`), consultas (`Modules/Loans/App/Services/LenderUserCategoryService.php:467`), capacidad (`Modules/Loans/App/Services/LenderUserCategoryService.php:477`). Basta UN criterio `false` para descartar ESE tier y saltar al siguiente.
...
**Perfilamiento = reordena, no excluye** (`ProfilingRulesService:30`): capacidad de endeudamiento + continuidad + consultas bajan el **nivel de probabilidad** (`alta/media/baja/muy baja/0%`, `Modules/Onboarding/App/Services/lenders/LenderRetrievalService.php:16-20`) vía `adjustProbability` (`Modules/Onboarding/App/Services/lenders/LenderRetrievalService.php:47-53`); delega el gate datacrédito-del-listado a `RiskCentralValidationService` (`Modules/Onboarding/App/Services/lenders/LenderRetrievalService.php:43`, solo si el allied ∈ frequencies y hay fila Experian-Acierta). **NO excluye** — el corte DURO real es la categoría (cupo) o la API externa (rt=1).
