...
- **BUG `min_income` NO-OP — MUERTO.** *Acá decía «vivo», y el mismo nodo lo desmiente más abajo (la re-verificación del 2026-08-28): quedó contradiciéndose. Verificado contra `origin/main` el 2026-09-16.* La columna del tier es `monthly_income` y `evaluateEligibility` **hoy la lee bien** (`Modules/Loans/App/Services/LenderUserCategoryService.php:513`: `$criteria['min_income'] = $salary >= $rule->monthly_income`). Mientras leyó `$rule->min_income` —atributo inexistente → `null` → `$salary >= null` es **siempre true** en PHP— el **piso de ingreso de la categoría no filtraba**; desde que se arregló, **endurece** la asignación. [MEMORY flow-reorg-y-mapa-atributos]
...
2. `evaluateEligibility:486` = **AND con early-exit**: ocupación (`Modules/Loans/App/Services/LenderUserCategoryService.php:493`) → edad `users.age∈[min,max]` (`Modules/Loans/App/Services/LenderUserCategoryService.php:508`) → `min_income` (`Modules/Loans/App/Services/LenderUserCategoryService.php:513`) → género (`Modules/Loans/App/Services/LenderUserCategoryService.php:516`) → continuidad (`Modules/Loans/App/Services/LenderUserCategoryService.php:519`) → **exige `$user->datacredito`** (`Modules/Loans/App/Services/LenderUserCategoryService.php:547`) → sub-checks de buró: neg12 `<=` (`Modules/Loans/App/Services/LenderUserCategoryService.php:580`), moras `currentNegativeCredits<=` (`Modules/Loans/App/Services/LenderUserCategoryService.php:585`), maturación `diffInMonths>=` (`Modules/Loans/App/Services/LenderUserCategoryService.php:596`), score `>=` (`Modules/Loans/App/Services/LenderUserCategoryService.php:602`), TC/vectores (`Modules/Loans/App/Services/LenderUserCategoryService.php:606`), consultas (`Modules/Loans/App/Services/LenderUserCategoryService.php:609`), capacidad (`Modules/Loans/App/Services/LenderUserCategoryService.php:627`). Basta UN criterio `false` para descartar ESE tier y saltar al siguiente.
...
`Modules/Loans/App/Services/LenderUserCategoryService.php:30`. Lector de referencia:
...
que pasó sino uno que **no se evaluó** (la evaluación corta en `Modules/Loans/App/Services/LenderUserCategoryService.php:425` tras los cinco criterios sin buró)
...
- **Cupo** = `calculateAvailableAmount` (`Modules/Loans/App/Services/LenderUserCategoryService.php:1026`): gross-up sobre el mínimo de **4 topes** —
  ...
  donde `availableFromProfile` = **PV francesa** de `(salario·(1−life_percentage) − pago_mensual_ajustado)` sobre `max_fee_number` cuotas a la `rate` del lender. La rama `debt_capacity_amount_validation==0` **omite** el término de perfil (`Modules/Loans/App/Services/LenderUserCategoryService.php:737` vs `Modules/Loans/App/Services/LenderUserCategoryService.php:739`). `salario` = `agildata > mareigua > EAV field 87 > 0` (`getSalary:384`); `pago_mensual_ajustado` reemplaza el dato viejo de Datacrédito por core bancario (`CreditopXDatacreditoAdjustmentService`).
- **Plazo** = `max_fee_number`; **FGA** = `FGA` %; **seguro de vida** = `life_percentage`.
- **Otorgación especial (DENTIX/DFS)** (`LenderSpecialGrantingService:37`): override por **buckets de score** (`Modules/Loans/App/Services/LenderSpecialGrantingService.php:194-216`: >770→15M/24c, 710-770→8M/24c, 650-709→5M/18c, ≤649→3M/12c), requiere estrato + ciudad de sucursal + ocupación; se capa al `max_amount` de la categoría.
...
- **LISTADO** (marketplace): `LenderRetrievalService::processRevolvingAndCreditopXLenders:675` fija `initial_fee_percentage = category.min_initial_fee` (`Modules/Loans/App/Http/Controllers/Customer/CreditopXQuotaController.php:756`) y el cupo mostrado `min(available_amount, loan_limit−already_used, max_amount)` (`Modules/Loans/App/Http/Controllers/Customer/CreditopXQuotaController.php:757`); si `available_amount < min_amount` → rechazo (`Modules/Loans/App/Http/Controllers/Customer/CreditopXQuotaController.php:776`). El **sello rt=2** (la card aparece iff hay categoría con cupo>0) se resuelve acá y en el listado v2 → ver **CreditopX** / **Motor de decisión**.

**Perfilamiento = reordena, no excluye** (`Modules/Onboarding/App/Services/lenders/ProfilingRulesService.php:28`): capacidad de endeudamiento + continuidad + consultas bajan el **nivel de probabilidad** (`alta/media/baja/muy baja/0%`, `Modules/Onboarding/App/Services/lenders/ProfilingRulesService.php:13-19`) vía `adjustProbability` (`Modules/Onboarding/App/Services/lenders/ProfilingRulesService.php:50-56`); delega el gate datacrédito-del-listado a `RiskCentralValidationService` (`Modules/Onboarding/App/Services/lenders/ProfilingRulesService.php:46`, solo si el allied ∈ frequencies y hay fila Experian-Acierta). **NO excluye** — el corte DURO real es la categoría (cupo) o la API externa (rt=1).
...
- **El bug NO-OP de `min_income` MURIÓ**: `evaluateEligibility` ahora lee `$rule->monthly_income`
  (`Modules/Loans/App/Services/LenderUserCategoryService.php:513`) — el piso de ingreso de las categorías **empezó a filtrar de
...
## Dónde mirar
- **Categoría / cupo (rt=2, legacy Loans — autoritativo)**: `Modules/Loans/App/Http/Controllers/Customer/CreditopXQuotaController.php` (`Modules/Loans/App/Http/Controllers/Customer/CreditopXQuotaController.php:66` cupo, `Modules/Loans/App/Http/Controllers/Customer/CreditopXQuotaController.php:268` categoría, `:331/348` scoring-block, `Modules/Loans/App/Http/Controllers/Customer/CreditopXQuotaController.php:362` special granting) · `Modules/Loans/App/Services/LenderUserCategoryService.php` (`Modules/Loans/App/Services/LenderUserCategoryService.php:74` getLenderUserCategory, `Modules/Loans/App/Services/LenderUserCategoryService.php:79` orden ASC, `Modules/Loans/App/Services/LenderUserCategoryService.php:105` 1er tier, `Modules/Loans/App/Services/LenderUserCategoryService.php:486` evaluateEligibility, `Modules/Loans/App/Services/LenderUserCategoryService.php:416` **min_income (BUG)**, `:697/737/739` cupo) · `LenderSpecialGrantingService.php:37` (buckets `Modules/Loans/App/Services/LenderSpecialGrantingService.php:194-216`) · `datacredito/CreditopXDatacreditoAdjustmentService.php` (ajuste de cuota mensual) · `LenderUserCategoryController.php`.
...
- **Datacrédito-cupo (gate antes de la categoría; mecánica en Motor de decisión)**: `Modules/Loans/App/Services/DatacreditoRuleEvaluator.php` (`Modules/Loans/App/Services/DatacreditoRuleEvaluator.php:19` evaluate, bypass CE&&84 `Modules/Loans/App/Services/DatacreditoRuleEvaluator.php:21`, sin regla skip→pass `Modules/Loans/App/Services/DatacreditoRuleEvaluator.php:25`, fail-closed `Modules/Loans/App/Services/DatacreditoRuleEvaluator.php:48`).
- **Tests (spec de comportamiento)**: `Modules/Loans/tests/Unit/LenderUserCategoryServiceMatchedByRuleTest.php`, `LenderSpecialGrantingServiceTest.php`, ~~`CreditopXDatacreditoAdjustmentServiceTest.php`~~ (**BORRADO** por CORE-431: usaba `RefreshDatabase` y podía vaciar la base compartida) · `tests/Feature/CreditopXQuotaControllerTest.php`.
...
|---|---|---|
| `CATEGORY_EVALUATION_START` | `Modules/Loans/App/Services/LenderUserCategoryService.php:92` | arrancó la evaluación del par (usuario, entidad) |
| `CATEGORY_EVALUATION_SPECIAL_CASE` | `Modules/Loans/App/Services/LenderUserCategoryService.php:68` | ⚠ **NO es special granting**: es el **bypass de venezolanos en Magnocell** (documento `CE` + lender 84) — `isVenezuelanMagnocell` → `handleVenezuelanCase`, con `event: venezuelan_magnocell_case`. La otorgación especial de DENTIX/DFS **no pasa por acá**: loguea como `QUOTA_CHECK_*` con `is_special_granting`, en `CreditopXQuotaController` |
| `CATEGORY_EVALUATION_APPROVED` | `Modules/Loans/App/Services/LenderUserCategoryService.php:131` | **cayó en categoría**, y con qué: `category_id`, `max_amount`, `available_from_lender`, `rules_evaluated`, `salary`, `continuity_months`, `datacredito_score` |
| `CATEGORY_RULE_REJECTED` | `Modules/Loans/App/Services/LenderUserCategoryService.php:159` | **el tier NO matcheó**: `rule_id`, `category_id` y sobre todo **`failed_criteria`** (`Modules/Loans/App/Services/LenderUserCategoryService.php:165`) — la lista exacta de criterios incumplidos (`negative_reports_last_12_months`, `overdue_accounts`, `score`) — más `criteria_details` |
| `FIELD_SCORING_COMPLETED` · `PAYMENT_CAPACITY_SCORING_COMPLETED` · `CATEGORY_RESOLUTION_BY_SCORE_COMPLETED` | `Modules/Loans/App/Services/LenderUserCategoryService.php:203` · `Modules/Loans/App/Services/LenderUserCategoryService.php:234` · `Modules/Loans/App/Services/LenderUserCategoryService.php:267` | el camino por **scoring**, el que corre cuando ningún tier matcheó |
...
⚠ Sale a nivel **`debug`** —el propio código dice «para no saturar» (`Modules/Loans/App/Services/LenderUserCategoryService.php:158`)—, a diferencia de todos los
