...
**El lado del backend son dos endpoints**, en `legacy-backend/Modules/Onboarding/App/Http/Controllers/NequiPaymentController.php`: `POST api/onboarding/loan-application/{user_request_id}/nequi-payment` para crear el cobro y `GET …/nequi-payment/status` para consultarlo — el que el front poll-ea. Sigue el mismo patrón que `WelliController`: servicio inyectado `private readonly`, span de `TracerService` **abierto al entrar y cerrado en TODOS los caminos de salida**, y `FormRequest` con getters tipados. El contrato completo para el front vive versionado en el repo, en `docs/lenders/nequi/CONTRATOS.md`.

**Dos detalles de la tarjeta que salieron de este mismo frente.** La línea «Te financiamos hasta X» (`frontend-monorepo/modules/loan-request-wizard/lenders-marketplace/src/components/lender-card/lender-card-max-financing.service.ts`) pide **dos condiciones y las dos hacen falta**: que sea **sólo Nequi** —ninguna otra entidad del comparador cambia— y que haya un **monto positivo y usable**, que sale de `credit_lines.max_amount`. La comparación por id vuelve a ser la deuda ya anotada, pero **queda en UN solo lugar** en vez de repartida por el render. Y el archivo **importa a los archivos hoja y no al barrel** a propósito: el barrel reexporta componentes que arrastran `@creditop/ui`, y esto tiene que poder cargarse en el entorno `node` de vitest.
...
Va acá y no en un nodo propio justamente porque no corre todavía. **Es un giro de modelo, no una
integración más**: en Recaudo BHD **CreditOp actúa como empresa recaudadora y el banco es un CANAL DE
PAGO, no un lender** — el banco consulta la deuda por cédula, notifica el pago y puede reversarlo, en
tiempo real. Es la primera vez que un tercero **entra** a preguntarle a CreditOp cuánto debe alguien.
El detalle (endpoints, arquitectura hexagonal, perfiles por `provider_key`) es de la rama y de su tarea;
lo que hay que saber ANTES de tocar cualquier cosa alrededor:

- ⚠ **La semántica de errores es al revés de lo habitual**: los errores de NEGOCIO responden **HTTP
  200** con un código `MTD*` en `CodigoRespuesta` (lo exige el contrato de BHD); los 4xx son solo
  transporte. Un monitor que cuente 5xx/4xx **no vería ningún error de negocio**.
- ⚠ **Montos enteros con decimales implícitos** (`13045` = RD$ 130,45) y la moneda en dos formatos según
  la dirección (consulta responde `DOP` alfabético; el request de pagos trae `000` numérico).

Fuente: Confluence · *Recaudo Referenciado y Arquitectura de Pagos en Tiempo Real* v2 (2026-08-03),
contrastado contra la rama.
