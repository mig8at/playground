...
- `application/app/Http/Controllers/Customer/SimulatorController.php:125-143` — lee las 2 filas de `settings`, OR, y `redirect()->away($this->urlService->init(...))` — o `inertia()->location(...)` si la petición trae `X-Inertia` (:137-138), que es lo que hace que el cutover funcione igual desde una navegación normal y desde un XHR de Inertia.
...
- `application/config/services.php:259-268` — `generate_services_bridge.base_url = LEGACY_BACKEND_BASE_URL`; `:278-280` — `new_frontend.base_url`.
...
- `legacy-backend/config/services.php:203-205` — `pre_approvals.base_url`; consumido en `legacy-backend/Modules/Loans/App/Actions/PreApprovalsAction.php:15,37`.
...
- **`LokiHandler` promueve `trace_id` y `span_id` a ETIQUETAS**, y en Loki cada valor distinto crea un stream: 959 streams cada 15 minutos *(medido el 2026-08-06; hoy ese conteo tiene que ser mucho menor, porque casi ninguna línea del monolito trae ya la etiqueta — ver el punto siguiente)*. Una consulta amplia sobre el lado Laravel (`{environment="production"}` a 30 días) devuelve 14,5 MB de series y expira. Hay que acotar siempre por `service_name`.

**Y una cuarta, la que más limita lo que se puede afirmar: sólo el 13 % de las líneas dice a qué solicitud pertenece** — y cuando lo dice, usa tres nombres de campo distintos (`context_user_request_id`, `context_userRequestId` en la integración BNPL, `context_request_id` en el cliente del MS de PDFs). Consecuencia: reconstruir el recorrido de una solicitud obliga a anclar y **expandir por `trace_id`**, y eso mezcla solicitudes de un mismo cliente. Medido en prod sobre 8 solicitudes de 7 comercios; el contraste es Corbeta **58 %** de líneas afirmables contra Credifamilia **4 %**, y no es volumen — Corbeta loguea menos, pero identifica lo que loguea. Detalle, mediciones y el arreglo en **F-102**. ⚠ **Y acá decía que el `trace_id` está en el «100 % de las líneas y esa parte funciona». Ya no.** Medido en prod el **2026-09-18**: de ~238.000 líneas de `legacy-backend` en 6 h, sólo **315 traen `trace_id` — el 0,13 %**; en `legacy-application` son 2.776 de 6.352 (~44 %). La causa está en el código y no en la configuración: `otel` es un **alias** de middleware (`legacy-backend/app/Http/Kernel.php:68`), **no** está en el stack global (`:16`) ni en el grupo `api` (`:33`), y lo aplican **diez** grupos de rutas, todos bajo `Modules/` (`Auth`, `BroadcastV1`, `DeviceLockingV1`, `LegalV1`, `Onboarding`, `OnboardingV2`, `RiskV2`, `UserRequestV1`, `UsersV1`). Todo lo que cae fuera —las rutas no modulares y **cada corrida de consola, cola o scheduler, que no pasan por ningún middleware HTTP**— escribe sin span, y el procesador del canal sólo pega el `trace_id` cuando hay uno grabando. **Consecuencia para soporte: el rodeo que recomienda F-102 —anclar y expandir por `trace_id`— hoy no funciona sobre `legacy-backend`.** Lo que falta sigue siendo la identidad de la solicitud dentro del log; lo que se perdió es el sustituto. (Y los **nombres de los spans** —`tracer->startSpan('Clase::metodo')`— tampoco viajan a Loki: viven en Tempo, que hoy no se consulta.)

**Desde el 2026-09-11 hay un instrumento más, y el nodo no lo mencionaba: el resumen de consultas por petición.** `legacy-backend/app/Logging/SlowQueryLogger.php`, enganchado como listener de `QueryExecuted` en `legacy-backend/app/Providers/EventServiceProvider.php:24`. Son **dos cosas distintas**, y sólo una está encendida por defecto:

- **La consulta lenta** («esta tardó 573 ms») encuentra el índice que falta. Pide umbral: `LOG_SLOW_QUERY_MS`, que **arranca en 0 = apagado** a propósito, porque un listado hace entre 98 y 129 consultas y sin umbral serían 129 líneas por petición.
- **El resumen por petición** («hizo 104 consultas, 57 repetidas») encuentra el **N+1**, que ninguna consulta individual delata — las 57 repeticiones eran de 2 ms cada una. Ese **sale siempre**, una vez por petición que toque la base.

Dos detalles del diseño que conviene no deshacer: **no registra los valores** (`bindings` lleva cédulas, teléfonos y correos; va la consulta con sus `?`, que además es lo que permite agrupar), y **escribe por `Log::channel('loki')` directo y no por el tracer**, porque —medido por su autor el 2026-09-11— una línea emitida desde *dentro* de un evento `QueryExecuted` no llega a ningún lado y su `catch` la traga sin error.

**Está vivo en producción y nadie lo ha mirado todavía.** Medido el 2026-09-18: **24.272 resúmenes en 6 h**. En esa misma ventana, **cinco peticiones** hicieron entre **20.507 y 35.455 consultas repetidas** cada una, de sólo **15 a 18 distintas**, con entre **66 y 187 segundos** de tiempo de base — y la dominante es siempre la misma, `select * from lender_transactions where id = ? limit 1`, hasta **4.241 veces en una sola petición**. Es un N+1 de libro que estaba invisible hasta que este instrumento entró: ver **F-239**.

Cómo se consulta (los campos del contexto se aplanan con `_`, porque `LokiHandler` serializa la línea entera como JSON):

    {service_name="legacy-backend", environment="production"} |~ `Consultas de la petici`
      | json | context_consultas_repetidas > 5000
      | line_format `{{.context_consultas_repetidas}} de {{.context_consultas_total}} · {{.context_consultas_ms}} ms · x{{.context_consultas_mas_repetida_veces}}: {{.context_consultas_mas_repetida}}`

⚠ **El resumen no se puede pegar a la petición que lo produjo:** se emite desde `app->terminating()`, o sea después de que el span cerró, así que **ninguno** de los 24.272 trae `trace_id` (medido). Dice *que* hay un N+1 y *cuál* es la consulta; no dice de qué endpoint salió.

**⚠ Hay un comentario en `main` que afirma lo contrario de todo esto, y es falso.** `legacy-backend/app/Services/Lenders/Nequi/NequiDiagnosticLog.php:13-15` abre diciendo que ese registro en tabla existe «porque en producción no hay otra vía: Grafana no recibe nada (el canal `loki` no se puede construir sin `GRAFANA_LOKI_ENDPOINT` y Laravel degrada en silencio al logger de emergencia, que escribe dentro del contenedor)». **El mecanismo que describe es real; la premisa no.** El mecanismo: `LokiHandler::__construct` declara `string $endpoint` **no nullable** (`legacy-backend/app/Logging/LokiHandler.php:30`), así que con la variable vacía tira `TypeError` al construir el canal, y `LogManager::get()` atrapa cualquier `Throwable` y devuelve el logger de emergencia, que escribe en `storage/logs/laravel.log` — dentro del contenedor. Eso es exactamente lo que pasa **si la variable falta**. Pero en producción no falta: medido en el stack `creditop` el **2026-09-18**, `legacy-backend` escribió **237.891 líneas en 6 h**, **215.568 de ellas por el canal `loki`** (el resto por `channel="production"`, que es el nombre que Laravel le pone al stack cuando `APP_ENV=production`), y **17.842 líneas que nombran Nequi en 24 h**. El comentario se escribió el **2026-09-15** (`ce8c0227`), día en que ese mismo servicio emitió **481.863** líneas: no caducó, nació falso.

Lo que costó creerle está en el mismo archivo: se construyó un segundo camino de diagnóstico que escribe en la tabla `logs` —que su propio docblock describe como «una tabla que no tiene retención ni índices»— para ver algo que Grafana ya mostraba. **La regla que deja esto:** antes de construir un camino de observabilidad paralelo porque «acá no se ve nada», contalo con `make trazador-acceso TARGET=prod QUERY='sum(count_over_time({service_name="…"} [6h]))'`. Son treinta segundos y el número decide.
