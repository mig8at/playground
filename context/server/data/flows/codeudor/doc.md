...
- **Qué entidades tienen catálogo cargado en cada ambiente.** Es dato, no código: se mide con una consulta a `lender_signing_documents` por ambiente (una migración de seed corrida en uno y no en otro cambia el comportamiento entero — ya pasó con Rent to Own).
