...
## Qué es
Repo `frontend-monorepo`: **1.778 archivos `.ts/.tsx/.js/.mjs`** en **27 workspaces pnpm** (re-contados el 2026-09-19; eran 1.198 y 21) (`pnpm-workspace.yaml` → `apps/*`, `packages/**`, `modules/**`). El producto real es **uno solo**: `apps/loan-request-wizard`, el wizard de originación que ve el cliente y el asesor. Todo lo demás es soporte (design system, storybook, landing Astro) o esqueleto vacío.
...
**(2026-09-19) Nodo RE-VERIFICADO entero.** 14 afirmaciones auditadas contra `main`, cero chequeos
débiles. **Dos quedaron obsoletas y una de ellas cambiaba el diagnóstico**: el `streamTimeout` pasó de
45 s a 120 s, perdió su condicional por ambiente, y —lo que más importa— **no acota lo que este nodo
suponía**. ✔ Se sostuvieron los hallazgos más finos: el `biome.json` de la app sigue incluyendo sólo
`app/**` (así que `utils/` sigue sin linter), `soft-update.tsx` sigue huérfana, y los buckets de
probabilidad siguen con `VERY_LOW: 5` corriendo a `ZERO` y `REJECTED`. Y el repo creció fuerte: de
1.198 a **1.778** archivos y de 21 a **27** workspaces.

## Antes de concluir
- **`lenders-v2` NO es SSE ni polling de cliente.** El progreso card-por-card sale de **Promises sin resolver serializadas por React Router v7** en el stream SSR (`available-lenders.tsx:213` → `:773` → `deferred-lender-resolution.adapter.ts:16`). El único polling real es **server-side y sólo para Credifamilia** (`fetch-lender-preapproval.ts:266`).
- ⚠ **`streamTimeout` ya no es 45 s ni depende del ambiente — y acota menos de lo que este nodo decía.** Verificado el 2026-09-19: vale **120 s, igual en todos los entornos** (`apps/loan-request-wizard/app/entry.server.tsx:26`), y el condicional por `APP_ENV` **se retiró** porque «daba 45 s fuera de staging sin razón medida». **Lo que más importa está en su comentario (`:20-25`) y contradice la lectura intuitiva:** este valor lo consume `encodeViaTurboStream` y acota **el encoding de promesas diferidas** más el stream de render SSR — **NO acota un loader o un action que hace `await`**. Está comprobado en QA: `otp-validation` **devolvió 200 a los 47 segundos** con el valor puesto en 45. Los timeouts de red son los del `HttpClient`. O sea que la tensión con el poll de Credifamilia (180 s) **sigue existiendo pero no por donde parecía**: el stream se corta, la llamada no.
- **Dos cookies de sesión distintas y fáciles de confundir**: `_session` (un guion bajo, secret `AUTH_SECRET`, dominio `.creditop.com`, 7 días — guarda `user`, `merchant-mode`, `alliedCountry`) en `utils/auth/session.server.ts:5`, vs `__session` (dos guiones, secret `SESSION_SECRET`) en `app/server/services/session.server.ts:27`, que sólo guarda el `sessionId` del form dinámico. Ambos exportan un símbolo llamado `sessionStorage`.
...
- ⚠ **Los buckets de probabilidad se renumeraron (2026-07-19).** Entró `VERY_LOW: 5` en
  `LENDER_PROBABILITY_BUCKETS` (`lender-resolution.service.ts:216-218`, re-verificadas el 2026-09-19), y eso **corrió `ZERO` de 5 a 6 y
...
- ✅ **`/modes` ya no existe, así que este bullet quedó sin objeto.** Decía que el redirect de entrada estaba comentado en `phone-number.tsx:134-136` pero la ruta seguía viva. Verificado el 2026-09-19: **ni el redirect comentado ni la ruta están** — `modes` no aparece en `routes.ts`. El archivo `phone-number.tsx` sigue existiendo; lo que se fue es el destino.
...
1. **`/:flow/:partner_hash/…`** — público, sin auth. `:flow` sólo admite **`ecommerce`** o **`self-service`** (`public-layout.tsx:20` redirige a `/` con cualquier otro valor). Es el camino del cliente final: `solicitar` → OTP → `personal-info` → `lenders` → CreditopX (confirmación, KYC, firma, Ábaco, IMEI/SmartPay).
2. **`/merchant/:partner_hash/…`** — el asesor. `default-layout.tsx:23` exige **Cognito** (`requireUserWithSession`) y siembra `session.set("alliedCountry", …)` en `:36`. Incluye el sub-árbol `merchant-dynamic-layout` con las 5 pantallas `request-{amount,phone,otp,personal-info,financial-info}`.
...
| `VITE_PREAPPROVALS_ENDPOINT` | **MS Go de pre-aprobaciones** (`POST /v1/preapprovals/check`) | **sólo `process.env`** (server-only) | `app/modules/allied-theme/application/get-lender-flow.ts:13` (⚠ acá se citaba *available-lenders* en su línea 148: el `alliedThemeRepository` **se sacó de esa ruta** y hoy vive en su propio módulo `allied-theme/`), `preapproval-retry.tsx:26` |
...
1. El loader (server) espera **`lenders-v2`** — llamada bloqueante con **timeout duro de 60s** (`loan-options.repository.ts:16`, endpoint en `:26`).
2. Filtra elegibles (`response_type !== STANDARD`, `available-lenders.tsx:150`) y **lanza una Promise por lender sin await** (`:209`). Los `is_fallback_lender` esperan a que todo el lote primario termine (`:231`). Las 4 variantes Welli **comparten una sola consulta** (`:200-229`).
...
SSR en `entry.server.tsx`: `renderToPipeableStream`, nonce aleatorio por request, CSP + HSTS aplicados en `security-headers.server.ts:135` (con `frame-ancestors 'none'`, `:98`) y **`streamTimeout` = 240s en staging / 45s en el resto** (`:15`), con `abort` a `streamTimeout + 1000` (`:105`).
...
**Rutas representativas**: `app/routes/loan-application-form/phone-number.tsx:70` (gate RD → flujo *dynamic*) · `app/routes/dynamic/request-amount.tsx:210` · `app/routes/loan-continue.tsx` (handoff/QR) · `app/routes/api/lender-results.tsx` (resource route) · `app/routes/form-preview.tsx` (preview dev del backend-driven-form) · `app/routes/health.tsx` · `app/routes/auth/callback.tsx`.
