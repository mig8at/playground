...
- **ML sin responder** (no «muerto»): ~20 campos `EX_*` de Experian se calculan y se tiran. En prod `makePrediction` **timeoutea a los 15 s** contra `profiler.inertia-production:8000` (medido 2026-08-06, 4 veces en la uReq 521997). ⚠ **No «cae al fallback de matrices»**: la cadena real es `new_then_legacy` —perfilador nuevo, y si falla, este H2O— y el nuevo ni siquiera se intenta porque `NEW_PROFILER_ML_HOST` no está puesta en prod. Ver **F-104**. Gran parte del reporte no decide nada hoy, pero el intento sí cuesta tiempo de respuesta y genera correos.
...
## Bitácora
- **2026-08-06** — Corregido de nuevo: el fallback del ML **no son las matrices**, es el otro perfilador (`new_then_legacy`); el nuevo está apagado por env faltante (F-104). Y el profiler ML no devuelve 404, **timeoutea a los 15 s** en prod (`ProfilerMLController::makePrediction`, `H2O_API_HOST`). 4 timeouts en una sola solicitud, cada uno con notificación por correo.
