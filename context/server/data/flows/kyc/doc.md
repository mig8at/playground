...
⚠ **Y desde el 2026-09-04 «el wizard no lo compra» hay que matizarlo: lo compra POR COMERCIO.** El wizard dejó de usar un camino fijo: si el comercio está en la lista del setting **`kyc_pipeline_allieds`**, va por el pipeline (v2); si no, por el flujo de siempre (v1). Es el mecanismo para encenderlo **de a un comercio y sin desplegar código** (`frontend-monorepo/apps/loan-request-wizard/app/routes/loan-application-form/kyc-pending-routing.ts`). **Medido en prod el 2026-09-18: la lista tiene UN comercio — `{"allieds":[91]}`, Mediarte, puesto el 2026-09-03.** Para los otros ~500 el wizard sigue en v1, así que lo que este nodo describe como el camino normal **es el camino de casi todos**, no el de todos.

⚠ **Ante la duda se sale por v1, y la asimetría es deliberada.** Setting ausente, JSON a medio escribir, el legacy sin responder, un código que ese build no conoce: todo cae a **v1**. No es simetría por prudencia genérica — es que **v1 es el que corre la cascada de burós dentro del legacy**, así que un comercio habilitado que caiga ahí por un error pasajero **hace KYC igual**; al revés no. ⚠ El costo de esa elección está medido y tiene nombre: **F-216** — con el setting ausente el endpoint da 500, el front cae a v1 **sin avisar**, y el ambiente parece sano mientras decide por otro camino.

### El backend desambiguaba `ONB005` desde mayo, y nadie lo consumía

`ONB005` es el código genérico de «falló la validación de identidad». Desde **mayo de 2026** (OBS-KYC-03) el backend lo acompaña con un **`error_subcode`** que dice cuál de las comprobaciones falló — y hasta el 2026-09-09 **nadie lo leía**: en todo el monorepo el campo aparecía **una sola vez, en un test**. O sea que durante meses el backend sabía exactamente qué había pasado y el cliente veía el mensaje genérico.

Hoy lo consume `frontend-monorepo/apps/loan-request-wizard/app/routes/loan-application-form/kyc-error-messages.ts`, que vive **fuera de la ruta** por el mismo motivo que `kyc-pending-routing.ts`: es una decisión con reglas propias, se prueba sin montar una petición, y el action ya estaba en el límite de complejidad que el repo acepta. Los subcódigos son un **enum cerrado** (`legacy-backend/Modules/Onboarding/App/Services/KycValidationOutcome.php`), así que la tabla del front **se puede mantener completa** — no es un mapeo abierto que se degrade con el tiempo.

⚠ **Y la copia de los textos vive en el FRONT a propósito: el backend dice QUÉ pasó, el front decide QUÉ se le dice al cliente.** El motivo es concreto y medido: el catálogo de códigos del backend tiene **42 mensajes en inglés**, del estilo «The lending product is not configured». Sin esa separación, un texto de desarrollador termina en la pantalla de una persona. Los subcódigos cubren la familia de documento (`DOCUMENT_NOT_FOUND`, `DOCUMENT_DUPLICATE`), la de fecha de expedición (`EXPEDITION_DATE_INVALID`, `EXPEDITION_DATE_MISMATCH`) y la de nacimiento (`BIRTH_DATE_INVALID`, `BIRTH_DATE_UNDERAGE`).

## Dónde mirar

### El cache de las centrales tiene UN dueño, y el motivo es una falla que no falla

`legacy-backend/Modules/RiskV2/App/Services/RiskCentralCacheManagementService.php` es dueño del cache de lectura de `risk_central_user_data`: su llave, su configuración y sus cuatro operaciones. ⚠ **Es un servicio de SISTEMA, no de dominio**: no expresa ninguna operación de negocio, no devuelve sobre `{code, message, data}` y **no decide nada del flujo** — contesta «¿está esta central cacheada y por cuánto?», «dame la fila», «cacheá esta fila», «borrá esta entrada». Si consultarlo, y qué hacer ante un fallo de cache, se queda en los orquestadores que lo llaman: `GetRiskCentralDataService` (lee y calienta) y `StoreRiskCentralDataService` (invalida después de escribir).

**Por qué existe, y por qué la lección generaliza:** esas cuatro operaciones más el constructor de la llave vivían **duplicadas** en los dos servicios consumidores, y el identificador de la llave estaba declarado en **dos constantes gemelas que podían separarse en silencio**. ⚠ **Una escritura que deja de invalidar lo que la lectura sirve no falla en ningún lado y no muestra ningún error**: simplemente se responde con datos viejos. Centralizar le da al cache **un solo dueño**, que es la única forma de que esa clase de falla no pueda volver.
...
- **Relaciones `User`** (application): `app/Models/User.php:303` (`datacredito()` por NOMBRE + latest) · `app/Models/User.php:315` (`tusDatos`, rc 2) · `app/Models/User.php:321` (`agildata`, 3) · `app/Models/User.php:327` (`mareigua`, 6) · `app/Models/User.php:333` (`aml`, 4) · `app/Models/User.php:339` (`ado`, por nombre `'Ado'`).
...
`front_status` (`Modules/Identity/App/Services/IdentityValidationService.php:332`/`Modules/Identity/App/Services/IdentityValidationService.php:338`), `back_status` (`Modules/Identity/App/Services/IdentityValidationService.php:416`/`Modules/Identity/App/Services/IdentityValidationService.php:423`) y `face_status` (`Modules/Identity/App/Services/IdentityValidationService.php:504`/`Modules/Identity/App/Services/IdentityValidationService.php:524`/`Modules/Identity/App/Services/IdentityValidationService.php:535`), cada
