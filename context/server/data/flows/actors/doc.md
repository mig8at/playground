...
- **`Entidad Comercio` no está sembrado** pero es el 3.er rol más usado del código (45 `hasRole`). Igual que arriba: existe sólo en la BD real. Y **representa a un LENDER, no a un comercio**: el usuario de SmartPay (`genaoalexander@gmail.com`) tiene `allied_id 277` (Carrefour) pero su identidad de alcance es `lender_id 160` (SmartPay). Verificado en prod el 2026-08-07.
- **Los filtros por rol escritos con `when()` fallan ABIERTO, no cerrado.** `CreditopXRequestsReportExport.php:76-91` (el reporte descargable de solicitudes originadas) arranca trayendo TODAS las solicitudes y las acota con dos `when()`: uno para `Superadmin comercio` y otro para `Admin comercio`, ambos por `allied_id`. Un `when()` con condición falsa **no aplica ningún filtro** — así que un rol no contemplado (justamente `Entidad Comercio`) recibe el universo entero. Reportado por SmartPay: en su Excel salían solicitudes de Mediarte, Pullman y el resto. El patrón se repite en cualquier alcance construido así: **el default de un `when()` encadenado es "ver todo"**, y en un panel multi-comercio eso es una fuga entre comercios. Al agregar un rol nuevo hay que auditar estos encadenados uno por uno.
