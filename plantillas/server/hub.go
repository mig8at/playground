...
type hub struct {
	db  *sql.DB
	mu  sync.Mutex
	sub map[string]map[chan evento]struct{} // sesión → suscriptores
}
...
func (h *hub) suscribir(sesion string) chan evento {
	ch := make(chan evento, 32)
	h.mu.Lock()
	defer h.mu.Unlock()
	if h.sub[sesion] == nil {
		h.sub[sesion] = map[chan evento]struct{}{}
	}
	h.sub[sesion][ch] = struct{}{}
	return ch
}

func (h *hub) desuscribir(sesion string, ch chan evento) {
	h.mu.Lock()
	defer h.mu.Unlock()
	if m := h.sub[sesion]; m != nil {
		delete(m, ch)
		if len(m) == 0 {
			delete(h.sub, sesion)
		}
...
func (h *hub) emitir(sesion, tipo string, payload any) {
	cuerpo, err := json.Marshal(payload)
	if err != nil {
		cuerpo = []byte("{}")
	}
	res, err := h.db.Exec(`INSERT INTO eventos (sesion_id, tipo, payload) VALUES (?,?,?)`,
		sesion, tipo, string(cuerpo))
	if err != nil {
		log.Printf("emitir %s/%s: %v", sesion, tipo, err)
		return
	}
...
// historial es el replay: todo lo que pasó en la sesión, para el que recién llega.
func (h *hub) historial(sesion string, desde int64) []evento {
	filas, err := h.db.Query(
		`SELECT id, tipo, payload, ts FROM eventos WHERE sesion_id = ? AND id > ? ORDER BY id`,
		sesion, desde)
	if err != nil {
		return nil
	}
